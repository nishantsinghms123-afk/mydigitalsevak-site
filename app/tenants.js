...
// Lists each tenant's signup contact number (orgs.contact_phone, migration 086)
// with tel: + WhatsApp links so a trial can actually be followed up on.
  ...
  function buildTenants(deps){
    ...
    function ProvisionForm({ onDone }){
      ...
      const submit=async()=>{
        ...
        try{
          await rpcCall('platform_provision_tenant',{ p_org_name:org.trim(), p_admin_email:email.trim().toLowerCase(), p_admin_name:name.trim(), p_plan:plan, p_phone:phone.trim()||null });
          setMsg(`Created ${org.trim()}. The admin signs in at /app with ${email.trim().toLowerCase()} (blank password → set password).`);
          setOrg(''); setEmail(''); setName(''); setPhone(''); onDone && onDone();
        }catch(e){
      ...
      return h`<div class="saas-card" style=${{padding:18,maxWidth:560,width:'100%'}}>
        ...
        <div style=${{display:'grid',gap:10}}>
          ...
          <input class="saas-input" type="tel" placeholder="Contact number (optional) — +91 98765 43210" value=${phone} onInput=${e=>setPhone(e.target.value)}/>
    ...
    function TenantsApp(){
      ...
      const visible=needle
        ?customers.filter(o=>[o.name,o.slug,o.admin_email,o.contact_phone,o.plan,o.status].some(v=>String(v||'').toLowerCase().includes(needle)))
        :customers;
      ...
      return h`<div style=${{padding:'26px 28px'}}>
        ...
        ${customers.length>0&&h`<div style=${{display:'flex',alignItems:'center',gap:10,marginBottom:14,flexWrap:'wrap'}}>
          <div style=${{position:'relative',flex:'1 1 260px',maxWidth:360}}>
            ...
            <input class="saas-input" style=${{width:'100%',paddingLeft:32}} placeholder="Search agency, email, phone, plan, status…" value=${q} onInput=${e=>setQ(e.target.value)}/>
          </div>
        ...
        <div class="saas-wrap" style=${{overflowX:'auto'}}>
          <table class="saas-table">
            <thead><tr>
              <th>Agency</th><th>Plan</th><th>Status</th><th>Clients</th><th>Seats</th><th>Trial ends</th><th>Admin</th><th>Phone</th><th>Joined</th><th></th>
            </tr></thead>
            <tbody>
              ${customers.length===0 && h`<tr><td colSpan=10 style=${{textAlign:'center',color:'var(--t3)',padding:'30px'}}>No customer agencies yet. Provision one or share your signup link.</td></tr>`}
              ${customers.length>0 && visible.length===0 && h`<tr><td colSpan=10 style=${{textAlign:'center',color:'var(--t3)',padding:'30px'}}>No agencies match “${q.trim()}”.</td></tr>`}
              ${visible.map(o=>{
                ...
                return h`<tr key=${o.id} style=${byOperator?{background:'rgba(220,38,38,.03)'}:undefined}>
                  ...
                  <td style=${{whiteSpace:'nowrap'}}>${o.contact_phone
                    ? h`<span style=${{display:'inline-flex',alignItems:'center',gap:7}}>
                        <a href=${'tel:'+o.contact_phone} style=${{color:'var(--t2)'}}>${o.contact_phone}</a>
                        <a href=${'https://wa.me/'+String(o.contact_phone).replace(/[^0-9]/g,'')} target="_blank" rel="noopener"
                           title="Message on WhatsApp" style=${{color:'var(--green)',fontSize:14,lineHeight:1}}><i class="ti ti-brand-whatsapp"></i></a>
                      </span>`
                    : h`<span style=${{color:'var(--t3)'}}>—</span>`}</td>
