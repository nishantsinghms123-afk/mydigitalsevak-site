...
(function(){
  function buildBilling(deps){
    const {React,h,createRoot,useState,useEffect,useRef,useCallback,useMemo,Av,EDate,ESelect,EText,EXPENSE_CATEGORIES,InvoiceTemplate,MONTHS,STATUS_STYLE,_loadFail,buildAgencySnapshot,buildClientSnapshot,calcInvoiceTotals,db,fmt,fmtDateLong,fmtINR,fmtMonthYear,generateAndUploadInvoicePdf,insertNotif,logActivity,numToIndianWords,ordinalDayLocal,rpcCall,sendInvoiceEmail,t,todayISO,uploadInvoiceAttachment,INVOICE_EMAIL_DEFAULTS,INVOICE_EMAIL_TOKENS,INVOICE_COPY_KEYS,mergeInvoiceTemplates} = deps;
...
function ClientInvoices({client,showToast}){
  ...
  // ✉ no longer fires immediately — it opens the composer so the copy can be
  // edited and the month's reports attached before anything leaves.
  const[emailingFor,setEmailingFor]=useState(null);
  const load=()=>{
    setLoading(true);
    Promise.all([
  ...
  // A brand-new invoice drops straight into the composer — that's the step
  // that used to happen silently behind your back.
  const onSaved=(saved)=>{setEditorOpen(false);setEditing(null);load();showToast(editing?'Invoice updated':'Invoice created');if(!editing&&saved)setEmailingFor(saved);};
  ...
  return h`<div style=${{marginTop:32,paddingTop:24,borderTop:'1px solid var(--bd)'}}>
      ...
      :h`<div>${rows.map(r=>{
        ...
        return h`<div key=${r.id} style=${{display:'grid',gridTemplateColumns:'auto 1fr auto auto',gap:12,alignItems:'center',padding:'10px 12px',background:'var(--bg)',border:'1px solid var(--bd)',borderRadius:8,marginBottom:6,cursor:'pointer'}} onClick=${()=>setViewing(r)}>
          ...
          <div style=${{display:'flex',alignItems:'center',gap:6}} onClick=${e=>e.stopPropagation()}>
            ...
            ${(r.status==='due'||r.status==='overdue'||r.status==='partial')&&h`<button class="icon-btn" style=${{padding:5}} onClick=${()=>setEmailingFor(r)} title="Compose invoice email"><i class="ti ti-mail-forward" style=${{color:'#ff00ee',fontSize:15}}></i></button>`}
    ...
    ${emailingFor&&h`<${InvoiceEmailComposer} invoice=${emailingFor} client=${client} agency=${agency} onClose=${()=>setEmailingFor(null)} onSent=${()=>{setEmailingFor(null);load();}} showToast=${showToast}/>`}
...
// Opens on the ✉ of an invoice row (and from "Email invoice" in the viewer).
// Everything the client will read is editable here before it goes out, and
// the month's reports can ride along as attachments.
//
// Copy layering (migration 085) — the modal prefills from the same chain the
// Edge Function resolves, so what you see here is what would have been sent:
//     INVOICE_EMAIL_DEFAULTS  <  agency_settings  <  clients  <  your edits
// Hitting Send passes the edited copy as a one-off override; the two "Save as
// default" buttons persist it to the agency row or this client's row instead.

// INVOICE_EMAIL_DEFAULTS / INVOICE_EMAIL_TOKENS / INVOICE_COPY_KEYS /
// mergeInvoiceTemplates come in through the bridge — they're shared with the
// agency-wide editor in Settings (AgencySettingsSection), so they live in
// index.html like every other shared helper.

function InvoiceEmailComposer({invoice,client,agency,onClose,onSent,showToast}){
  const agencyTpl=agency?.invoice_email_template||null;
  const clientTpl=client?.invoice_email_template||null;
  const defaultTo=(invoice?.client_snapshot?.contact_email||client?.contact_email||'').trim();
  const[to,setTo]=useState(defaultTo);
  const[form,setForm]=useState(()=>{
    const merged=mergeInvoiceTemplates(INVOICE_EMAIL_DEFAULTS,agencyTpl,clientTpl);
    // The note box falls through to the invoice's own note when no template
    // sets one — mirrors the server, so what you read here is what would ship.
    const tplNote=clientTpl?.note??agencyTpl?.note;
    merged.note=(tplNote===undefined||tplNote===null)?(invoice?.notes||''):String(tplNote);
    return merged;
  });
  const[showMore,setShowMore]=useState(false);
  const[sending,setSending]=useState(false);
  const[savingTpl,setSavingTpl]=useState('');
  ...
  // Saved report PDFs for this brand — social first, then SEO, newest first.
  useEffect(()=>{
    if(!client?.id)return;
    let alive=true;
    Promise.all([
      db('monthly_reports',`&client_id=eq.${client.id}&deleted_at=is.null&pdf_url=not.is.null&select=id,month,pdf_url&order=month.desc&limit=12`).catch(()=>[]),
      db('seo_reports',`&client_id=eq.${client.id}&deleted_at=is.null&pdf_url=not.is.null&select=id,period_type,period_key,period_end,pdf_url&order=period_end.desc&limit=12`).catch(()=>[]),
    ]).then(([social,seo])=>{
      if(!alive)return;
      const list=[
        ...(social||[]).map(r=>({
          key:'social-'+r.id,kind:'Social report',
          label:fmtMonthYear(r.month),
          filename:`${(client.name||'Report').replace(/[^\w.\- ]+/g,'_')} - Social report - ${fmtMonthYear(r.month)}.pdf`,
          url:r.pdf_url,
        })),
        ...(seo||[]).map(r=>({
          key:'seo-'+r.id,kind:'SEO report',
          label:r.period_type==='weekly'?`Week ending ${fmt(r.period_end)}`:fmtMonthYear(r.period_end),
          filename:`${(client.name||'Report').replace(/[^\w.\- ]+/g,'_')} - SEO report - ${r.period_key||fmt(r.period_end)}.pdf`,
          url:r.pdf_url,
        })),
      ];
      setReports(list);
  ...
  // Click a token chip to drop it at the cursor of whichever field you last
  // touched — beats making people memorise the placeholder names.
  const insertToken=(tok)=>{
    const el=lastField.current;
    if(!el||!el.dataset?.k){showToast?.('Click into a field first, then pick a tag');return;}
    const k=el.dataset.k;
    const start=el.selectionStart??el.value.length;
    const end=el.selectionEnd??el.value.length;
    const next=el.value.slice(0,start)+tok+el.value.slice(end);
    set(k,next);
    requestAnimationFrame(()=>{try{el.focus();el.setSelectionRange(start+tok.length,start+tok.length);}catch(_){}});
  };
  ...
  const addFiles=(list)=>{
    ...
    if(tooBig.length)showToast?.(`${tooBig.length} file(s) over 15MB were skipped`);
    setFiles(prev=>[...prev,...incoming.filter(f=>f.size<=15*1024*1024)]);
  };

  // What gets mailed for THIS send — exactly what's on screen, blanks included.
  const copyPayload=()=>Object.fromEntries(INVOICE_COPY_KEYS.map(k=>[k,form[k]??'']));

  // What gets PERSISTED as a reusable default — not the same thing. Two keys
  // need holding back: `cc` because blank means "no standing CC" rather than
  // "blank it out", and `note` because it's prefilled from this one invoice —
  // saving it would stamp August's note onto every future month.
  const templatePayload=()=>{
    const out={};
    INVOICE_COPY_KEYS.forEach(k=>{
      const v=String(form[k]??'');
      if(k==='cc'&&!v.trim())return;
      if(k==='note'&&(!v.trim()||v===String(invoice?.notes||'')))return;
      out[k]=v;
    });
    return out;
  };

  const saveTemplate=async(scope)=>{
    setSavingTpl(scope);
    try{
      const tpl=templatePayload();
      if(scope==='client'){
        const res=await rpcCall('client_update',{p_client_id:client.id,p_patch:{invoice_email_template:tpl}});
        // client_update v2 (migration 076) reports dropped keys instead of
        // silently lying "Saved" — surface it, it means 085 isn't applied.
        if(Array.isArray(res?.skipped)&&res.skipped.includes('invoice_email_template')){
          showToast?.('Not saved — run migration 085 in Supabase first');return;
        }
        showToast?.(`Saved as the default for ${client.name}`);
      }else{
        await rpcCall('agency_settings_update',{p_patch:{invoice_email_template:tpl}});
        showToast?.('Saved as the agency-wide default');
      }
      logActivity({action:'update',table_name:scope==='client'?'clients':'agency_settings',record_id:String(scope==='client'?client.id:1),record_label:'Invoice email template',changes:null});
    }catch(e){
      console.error('[invoice-email] template save failed',e);
      showToast?.('Could not save the template: '+(e?.message||e));
    }finally{setSavingTpl('');}
  };

  const resetCopy=()=>{
    setForm(mergeInvoiceTemplates(INVOICE_EMAIL_DEFAULTS,agencyTpl,clientTpl));
    showToast?.('Copy reset to the saved default');
  };

  const send=async()=>{
    if(!to.trim()){showToast?.('Add a recipient email');return;}
    if(!String(form.subject||'').trim()){showToast?.('Subject cannot be empty');return;}
    setSending(true);
    try{
      const attachments=[];
      if(attachInvoice){
        setProgress('Rendering the invoice PDF…');
        const out=await generateAndUploadInvoicePdf(invoice);
        if(out?.url)attachments.push({filename:out.filename,url:out.url});
        else showToast?.('Invoice PDF could not be attached — sending without it');
      }
      reports.filter(r=>picked[r.key]).forEach(r=>attachments.push({filename:r.filename,url:r.url}));
      for(let i=0;i<files.length;i++){
        setProgress(`Uploading ${files[i].name}…`);
        const out=await uploadInvoiceAttachment(files[i],invoice.id);
        if(out?.url)attachments.push({filename:out.filename,url:out.url});
        else showToast?.(`Could not attach ${files[i].name}${out?.error==='too_large'?' (over 15MB)':''}`);
      }
      setProgress('Sending…');
      const res=await sendInvoiceEmail(invoice.id,{
        to:to.trim(),
        cc:String(form.cc||'').trim()||undefined,
        template:copyPayload(),
        attachments,
      });
      if(res.ok){
        showToast?.(`Sent to ${res.to||to.trim()}${res.attachments?` · ${res.attachments} attachment${res.attachments===1?'':'s'}`:''} ✓`);
        logActivity({action:'update',table_name:'invoices',record_id:String(invoice.id),record_label:invoice.invoice_number+' · emailed',changes:null});
        onSent?.();
      }else if(res.error==='no email on file for this client'){
      ...
      showToast?.('Email failed: '+(e?.message||e));
    }finally{setSending(false);setProgress('');}
  };

  // Plain function, not a component — a component defined inside render gets a
  // fresh identity every keystroke and the textarea would lose focus.
  const TA=({label,k,rows,hint,mono})=>h`<div class="fi-group" key=${k}>
    <div class="fi-lbl">${label}</div>
    <textarea class="fi" data-k=${k} rows=${rows||2}
      value=${form[k]??''}
      onInput=${e=>set(k,e.target.value)}
      onFocus=${e=>{lastField.current=e.target;}}
      style=${{resize:'vertical',lineHeight:1.55,fontFamily:mono?"'JetBrains Mono',monospace":'inherit',fontSize:mono?12:13}}></textarea>
    ${hint&&h`<div style=${{fontSize:11,color:'var(--t3)',marginTop:4}}>${hint}</div>`}
  </div>`;

  const attachCount=(attachInvoice?1:0)+reports.filter(r=>picked[r.key]).length+files.length;

  return h`<div class="modal" onClick=${e=>{if(e.target===e.currentTarget&&!sending)onClose();}}>
    <div class="modal-box" style=${{maxWidth:720}}>
      <div class="modal-head">
        <div>
          <div style=${{fontSize:16,fontWeight:600}}>Send invoice email</div>
          <div style=${{fontSize:12,color:'var(--t3)',marginTop:2}}>${invoice?.invoice_number} · ${client?.name||''} · ₹${fmtINR(Number(invoice?.amount)||0,false)}</div>
        </div>
      ...
      <div class="modal-body">
        ...
        <div class="fi-grid fi-group">
          <div>
            <div class="fi-lbl">To</div>
            <input class="fi" type="text" value=${to} onInput=${e=>setTo(e.target.value)} placeholder="billing@brand.com"/>
          </div>
          <div>
            <div class="fi-lbl">CC (optional)</div>
            <input class="fi" type="text" data-k="cc" value=${form.cc||''} onInput=${e=>set('cc',e.target.value)} onFocus=${e=>{lastField.current=e.target;}} placeholder="accounts@brand.com"/>
          </div>
        </div>

        <div class="fi-group">
          <div class="fi-lbl">Subject</div>
          <input class="fi" type="text" data-k="subject" value=${form.subject||''} onInput=${e=>set('subject',e.target.value)} onFocus=${e=>{lastField.current=e.target;}}/>
        </div>

        <div style=${{display:'flex',flexWrap:'wrap',gap:5,margin:'2px 0 14px'}}>
          <span style=${{fontSize:11,color:'var(--t3)',alignSelf:'center',marginRight:2}}>Insert:</span>
          ${INVOICE_EMAIL_TOKENS.map(([tok,desc])=>h`<button key=${tok} type="button" title=${desc}
            onClick=${()=>insertToken(tok)}
            style=${{padding:'3px 8px',fontSize:10.5,fontFamily:"'JetBrains Mono',monospace",borderRadius:6,border:'1px solid var(--bd)',background:'var(--bg2)',color:'var(--t2)',cursor:'pointer'}}>${tok}</button>`)}
        </div>

        ${TA({label:'Greeting (large headline)',k:'greeting',rows:1})}
        ${TA({label:'Opening paragraph',k:'intro',rows:3})}
        ${TA({label:'Highlighted note (leave blank to hide)',k:'note',rows:3,hint:"Prefilled from the invoice's own note. Good place to summarise what the attached reports cover."})}

        <button class="btn-sec" style=${{padding:'6px 11px',fontSize:11.5,marginBottom:showMore?14:4}} onClick=${()=>setShowMore(v=>!v)}>
          <i class=${'ti ti-chevron-'+(showMore?'up':'down')}></i>${showMore?'Hide':'Edit'} the rest of the email
        </button>
        ${showMore&&h`<div style=${{paddingLeft:12,borderLeft:'2px solid var(--bd)'}}>
          ${TA({label:'Portal button label',k:'cta_label',rows:1})}
          ${TA({label:'Payment-timing line',k:'due_line',rows:2})}
          ${TA({label:'"Anything off?" box',k:'reply_note',rows:2})}
          ${TA({label:'Sign-off',k:'signoff',rows:1})}
          ${TA({label:'Sign-off name',k:'signoff_name',rows:1})}
          ${TA({label:'Footer line (under the GSTIN)',k:'footer_note',rows:2})}
          <div style=${{fontSize:11,color:'var(--t3)',marginTop:-4,marginBottom:10}}>
            ${'The invoice table, GST/TDS breakdown, bank & UPI details and the signed-contract link are always included — they\'re built from the invoice itself, not from this copy.'}
          </div>
        </div>`}

        <div style=${{marginTop:18,paddingTop:16,borderTop:'1px solid var(--bd)'}}>
          <div style=${{fontSize:13,fontWeight:600,color:'var(--t1)',marginBottom:2}}>Attachments${attachCount?h` <span style=${{color:'#ff00ee'}}>· ${attachCount}</span>`:''}</div>
          <div style=${{fontSize:11.5,color:'var(--t3)',marginBottom:12}}>Files are uploaded to your report storage, then attached to the email.</div>

          <label style=${{display:'flex',alignItems:'center',gap:9,padding:'9px 11px',border:'1px solid var(--bd)',borderRadius:8,marginBottom:6,cursor:'pointer',fontSize:12.5}}>
            <input type="checkbox" checked=${attachInvoice} onChange=${e=>setAttachInvoice(e.target.checked)}/>
            <i class="ti ti-file-invoice" style=${{color:'#ff00ee',fontSize:15}}></i>
            <span style=${{flex:1}}>Invoice PDF <span style=${{color:'var(--t3)'}}>— generated fresh when you hit Send</span></span>
          </label>

          ${reportsLoading?h`<div style=${{fontSize:12,color:'var(--t3)',padding:'6px 2px'}}>Looking for saved reports…</div>`
            :reports.length===0?h`<div style=${{fontSize:11.5,color:'var(--t3)',padding:'6px 2px'}}>No saved report PDFs for ${client?.name||'this client'} yet — generate one from the Reports or SEO tab and it'll show up here.</div>`
            :reports.map(r=>h`<label key=${r.key} style=${{display:'flex',alignItems:'center',gap:9,padding:'9px 11px',border:'1px solid var(--bd)',borderRadius:8,marginBottom:6,cursor:'pointer',fontSize:12.5}}>
              <input type="checkbox" checked=${!!picked[r.key]} onChange=${e=>setPicked(p=>({...p,[r.key]:e.target.checked}))}/>
              <i class=${'ti '+(r.kind==='SEO report'?'ti-trending-up':'ti-chart-bar')} style=${{color:'var(--t3)',fontSize:15}}></i>
              <span style=${{flex:1,minWidth:0}}>${r.kind} <span style=${{color:'var(--t3)'}}>· ${r.label}</span></span>
            </label>`)}
          ...
          <div
            ...
            style=${{marginTop:8,padding:'14px 12px',border:'1px dashed '+(dragOver?'#ff00ee':'var(--bd)'),background:dragOver?'rgba(255,0,238,.06)':'transparent',borderRadius:8,textAlign:'center',cursor:'pointer',fontSize:12,color:'var(--t3)'}}>
            <i class="ti ti-paperclip" style=${{fontSize:15,marginRight:6}}></i>Drop files here or click to browse <span style=${{opacity:.7}}>· up to 15MB each</span>
          </div>
          <input ref=${fileInput} type="file" multiple style=${{display:'none'}} onChange=${e=>{addFiles(e.target.files);e.target.value='';}}/>
          ${files.map((f,i)=>h`<div key=${f.name+i} style=${{display:'flex',alignItems:'center',gap:9,padding:'7px 11px',background:'var(--bg2)',borderRadius:8,marginTop:6,fontSize:12}}>
            <i class="ti ti-file" style=${{color:'var(--t3)',fontSize:14}}></i>
            <span style=${{flex:1,minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>${f.name}</span>
            <span style=${{color:'var(--t3)',fontSize:11}}>${(f.size/1024/1024).toFixed(1)} MB</span>
            <button class="icon-btn" style=${{padding:3}} onClick=${()=>setFiles(list=>list.filter((_,j)=>j!==i))}><i class="ti ti-x" style=${{fontSize:13}}></i></button>
          </div>`)}
        </div>

      </div>
      <div class="modal-foot" style=${{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
        <button class="btn-sec" style=${{padding:'7px 11px',fontSize:11.5}} onClick=${()=>saveTemplate('client')} disabled=${!!savingTpl||sending} title=${'Reuse this wording for every future '+(client?.name||'client')+' invoice'}>
          ${savingTpl==='client'?'Saving…':'Save for '+(client?.name||'client')}
        </button>
        <button class="btn-sec" style=${{padding:'7px 11px',fontSize:11.5}} onClick=${()=>saveTemplate('agency')} disabled=${!!savingTpl||sending} title="Reuse this wording for every client's invoice email">
          ${savingTpl==='agency'?'Saving…':'Save as agency default'}
        </button>
        <button class="btn-sec" style=${{padding:'7px 11px',fontSize:11.5}} onClick=${resetCopy} disabled=${sending}>Reset</button>
        <div style=${{flex:1,minWidth:8,fontSize:11.5,color:'var(--t3)',textAlign:'right'}}>${progress}</div>
        <button class="btn-sec" onClick=${onClose} disabled=${sending}>Cancel</button>
        <button class="btn-pri" onClick=${send} disabled=${sending}>
          ${sending?h`<i class="ti ti-loader-2 spinner"></i>Sending…`:h`<i class="ti ti-send"></i>Send${attachCount?` · ${attachCount} file${attachCount===1?'':'s'}`:''}`}
        </button>
      </div>
    </div>
  </div>`;
...
function StatementOfAccount({client,invoices,agency,onClose,showToast}){
  ...
  return h`<div class="modal" onClick=${e=>{if(e.target===e.currentTarget)onClose();}}>
    ...
    // ── Billable time → invoice lines (workstream I · migration 099) ──
// "Add billable time" pulls the client's uninvoiced, billable tracked time for a
// period and previews it as ORDINARY line items (hours × hourly rate). GST/TDS
// maths, invoice numbering, the PDF and the email are untouched — this only
// appends rows to form.line_items. Each generated row carries `_time` (the time
// entry ids); that key is stripped before saving and used to mark those entries
// invoiced once the invoice itself exists. Cancelling or deleting the invoice
// releases them again (DB trigger in 099), so nothing is ever billed twice.
// Fails open: if migration 099 isn't applied the button simply never appears.
const isMissingTimeRpc=(e)=>/PGRST202|could not find the function|does not exist|404/i.test(String((e&&(e.code||''))+' '+((e&&(e.message||e.details))||'')));
const TIME_RPC={ok:null};   // null = untested · true = available · false = not applied
const timeErr=(e)=>{
  const s=String((e&&(e.code||''))+' '+((e&&(e.message||e.details))||''));
  if(/time\.already_invoiced/.test(s))return 'Some of that time is already on another invoice.';
  if(/time\.client_mismatch/.test(s))return 'That time belongs to a different client.';
  if(/time\.not_billable/.test(s))return 'That time is marked non-billable.';
  if(/time\.running/.test(s))return 'A timer is still running on that time.';
  if(/invoice\.not_active/.test(s))return 'This invoice is cancelled or in the trash.';
  if(/auth\.forbidden|forbidden/.test(s))return 'Only an admin or manager can bill tracked time.';
  if(/not_found/.test(s))return 'That invoice or time entry is gone — reload.';
  return (e&&e.message)||'Something went wrong';
};
const round2=(n)=>Math.round((Number(n)||0)*100)/100;
const fmtMins=(n)=>{n=Math.round(Number(n)||0);const hh=Math.floor(n/60),mm=n%60;return n<=0?'0m':(hh?hh+'h':'')+(hh&&mm?' ':'')+(mm?mm+'m':'');};
const TIME_GROUPS=[['task','One line per task'],['task_member','Task × person'],['member','One line per person'],['total','A single line']];

function BillableTimeModal({client,agency,periodStart,onClose,onAdd,showToast}){
  const seed=periodStart||todayISO();
  const mStart=String(seed).slice(0,7)+'-01';
  const mEnd=(()=>{const d=new Date(mStart+'T00:00:00');d.setMonth(d.getMonth()+1);d.setDate(0);return`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;})();
  const[from,setFrom]=useState(mStart);
  const[to,setTo]=useState(mEnd);
  const[group,setGroup]=useState('task');
  ...
  const load=useCallback(async()=>{
    ...
    try{
      const d=await rpcCall('time_billable_summary',{p_client_id:client.id,p_from:from,p_to:to,p_group:group,p_only_approved:onlyApproved});
      TIME_RPC.ok=true;
      setData(d||null);
      setRows((d&&Array.isArray(d.lines)?d.lines:[]).map((l,i)=>({
        key:String(l.key||i),on:!l.missing_rate,description:l.description||'Professional services',
        hours:Number(l.hours)||0,minutes:Number(l.minutes)||0,rate:l.rate==null?'':String(l.rate),
        entry_ids:Array.isArray(l.entry_ids)?l.entry_ids:[],missing:!!l.missing_rate,
      })));
    }catch(e){
      if(isMissingTimeRpc(e)){TIME_RPC.ok=false;setErr('missing');}
      else setErr(timeErr(e));
    }finally{setLoading(false);}
  },[client.id,from,to,group,onlyApproved]);
  useEffect(()=>{load();},[load]);
  const setRow=(k,patch)=>setRows(rs=>rs.map(r=>r.key===k?{...r,...patch}:r));
  const amountOf=(r)=>round2(r.hours*(Number(r.rate)||0));
  const chosen=rows.filter(r=>r.on&&Number(r.rate)>0&&r.hours>0);
  const total=chosen.reduce((s,r)=>s+amountOf(r),0);
  const add=()=>{
    if(!chosen.length){alert('Tick at least one line with an hourly rate.');return;}
    onAdd(chosen.map(r=>({
      description:r.description.trim()||'Professional services',
      hsn_sac:agency?.default_hsn_sac||'998314',
      qty:r.hours,rate:Number(r.rate),amount:amountOf(r),
      _time:r.entry_ids,
    })));
  };
  return h`<div class="modal" onClick=${e=>{if(e.target===e.currentTarget)onClose();}}>
    <div class="modal-box" style=${{maxWidth:780}}>
      <div class="modal-head">
        <div>
          <div style=${{fontSize:16,fontWeight:600}}>Add billable time</div>
          <div style=${{fontSize:12,color:'var(--t3)',marginTop:2}}>Uninvoiced billable time tracked for ${client.name}. Lines are added as normal invoice lines — GST is worked out exactly as always.</div>
        </div>
        <button class="icon-btn" onClick=${onClose}><i class="ti ti-x"></i></button>
      </div>
      <div class="modal-body">
        <div class="fi-grid fi-group" style=${{gridTemplateColumns:'1fr 1fr 1fr'}}>
          <div><div class="fi-lbl">From</div><input class="fi" type="date" value=${from} onInput=${e=>setFrom(e.target.value)}/></div>
          <div><div class="fi-lbl">To</div><input class="fi" type="date" value=${to} onInput=${e=>setTo(e.target.value)}/></div>
          <div><div class="fi-lbl">Group as</div>
            <select class="fi fi-select" value=${group} onChange=${e=>setGroup(e.target.value)}>
              ${TIME_GROUPS.map(([k,l])=>h`<option key=${k} value=${k}>${l}</option>`)}
            </select></div>
        </div>
        <label style=${{display:'inline-flex',alignItems:'center',gap:6,fontSize:12,color:'var(--t2)',marginBottom:10}}>
          <input type="checkbox" checked=${onlyApproved} onChange=${e=>setOnlyApproved(e.target.checked)} style=${{accentColor:'#ff00ee'}}/>
          Only time on approved timesheets
        </label>

        ${loading?h`<div style=${{padding:'22px 0',textAlign:'center',color:'var(--t3)',fontSize:13}}>Loading tracked time…</div>`
          :err==='missing'?h`<div style=${{padding:'22px 0',textAlign:'center',color:'var(--t3)',fontSize:13}}>Time tracking isn't switched on yet.</div>`
          :err?h`<div class="err"><i class="ti ti-alert-circle"></i>${err}</div>`
          :rows.length===0?h`<div style=${{padding:'22px 0',textAlign:'center',color:'var(--t3)',fontSize:13,border:'1px dashed var(--bd)',borderRadius:8}}>
              No uninvoiced billable time for ${client.name} in this period.</div>`
          :h`<div>
            <table class="inv-line-tbl">
              <thead><tr>
                <th style=${{width:28}}></th>
                <th>Description</th>
                <th class="num" style=${{width:80}}>Hours</th>
                <th class="num" style=${{width:110}}>Rate (₹)</th>
                <th class="num" style=${{width:110}}>Amount</th>
              </tr></thead>
              <tbody>
                ${rows.map(r=>h`<tr key=${r.key} style=${r.on?null:{opacity:.5}}>
                  <td style=${{textAlign:'center'}}><input type="checkbox" checked=${r.on} onChange=${e=>setRow(r.key,{on:e.target.checked})} style=${{accentColor:'#ff00ee'}}/></td>
                  <td><input value=${r.description} onInput=${e=>setRow(r.key,{description:e.target.value})}/>
                    <div style=${{fontSize:10.5,color:'var(--t3)',padding:'2px 6px'}}>${fmtMins(r.minutes)} tracked${r.missing?' · no hourly rate set — type one':''}</div></td>
                  <td class="num">${r.hours}</td>
                  <td class="num"><input type="number" min="0" step="50" value=${r.rate} placeholder="0" onInput=${e=>setRow(r.key,{rate:e.target.value})}/></td>
                  <td class="amt">${fmtINR(amountOf(r))}</td>
                </tr>`)}
              </tbody>
            </table>
            <div style=${{marginTop:12,padding:'10px 12px',background:'var(--bg2)',borderRadius:8,fontSize:12.5,color:'var(--t2)',display:'flex',justifyContent:'space-between',gap:10,flexWrap:'wrap'}}>
              <span>${chosen.length} line${chosen.length===1?'':'s'} · ${fmtMins(chosen.reduce((s,r)=>s+r.minutes,0))} of tracked time</span>
              <span>Adds <strong style=${{color:'#ff00ee'}}>₹${fmtINR(total,false)}</strong> before GST</span>
            </div>
            ${data&&data.missing_rate_minutes>0&&h`<div style=${{marginTop:8,fontSize:11.5,color:'#B45309'}}>
              ${fmtMins(data.missing_rate_minutes)} has no hourly rate yet — set rates in Timesheets → Rates, or type a rate above.</div>`}
          </div>`}
      </div>
      <div class="modal-foot">
        <button class="btn-sec" onClick=${onClose}>Cancel</button>
        <button class="btn-pri" disabled=${loading||!chosen.length} onClick=${add}>Add ${chosen.length||''} line${chosen.length===1?'':'s'}</button>
      </div>
    </div>
  </div>`;
}
    ...
    // ── InvoiceEditor ──
function InvoiceEditor({client,agency,existing,onClose,onSaved,autoTime}){
  ...
  // Billable time (migration 099) — see BillableTimeModal above.
  const[timeOpen,setTimeOpen]=useState(false);
  const[timeAvail,setTimeAvail]=useState(TIME_RPC.ok);
  const[linkedTime,setLinkedTime]=useState(null);   // time already marked against this invoice
  const[linkBusy,setLinkBusy]=useState(false);
  const loadLinkedTime=useCallback(async()=>{
    if(!existing?.id||TIME_RPC.ok===false)return;
    try{
      const d=await rpcCall('time_invoice_entries',{p_invoice_id:existing.id});
      TIME_RPC.ok=true;setTimeAvail(true);setLinkedTime(d||null);
    }catch(e){
      if(isMissingTimeRpc(e)){TIME_RPC.ok=false;setTimeAvail(false);}
      else if(/auth\.forbidden|forbidden/.test(String(e?.code||'')+' '+String(e?.message||'')))setTimeAvail(false);
    }
  },[existing?.id]);
  useEffect(()=>{loadLinkedTime();},[loadLinkedTime]);
  useEffect(()=>{if(autoTime&&TIME_RPC.ok!==false)setTimeOpen(true);},[autoTime]);
  const addTimeLines=(lines)=>{
    setTimeOpen(false);
    if(!lines||!lines.length)return;
    setForm(f=>({...f,line_items:[...f.line_items,...lines]}));
  };
  const releaseTime=async()=>{
    if(!existing?.id||!confirm('Release this tracked time from the invoice? It becomes billable again — the invoice lines stay as they are.'))return;
    setLinkBusy(true);
    try{
      await rpcCall('time_unmark_invoiced',{p_invoice_id:existing.id});
      setLinkedTime({count:0,minutes:0,entries:[]});
    }catch(e){alert('Could not release the time: '+timeErr(e));}
  ...
  const submit=async()=>{
    ...
    try{
      ...
      const payload={
        ...
        // `_time` (tracked-entry ids on a billable-time line) is a local marker —
        // it never reaches the invoice row, the PDF or the email.
        line_items:form.line_items.map(({_time,...it})=>({...it,qty:Number(it.qty)||0,rate:Number(it.rate)||0,amount:(Number(it.qty)||0)*(Number(it.rate)||0)})),
      ...
      if(existing){
        ...
        // The email is NOT fired here any more. It used to go out silently the
        // instant an invoice was created, which meant the client got an
        // attachment-less mail before anyone could add the month's reports —
        // and a second one if you then used the composer. ClientInvoices now
        // opens InvoiceEmailComposer on save instead, so the send is one
        // deliberate click with the reports attached.
      }
      // Now that the invoice exists, stamp the tracked time as invoiced so it
      // can't be billed twice. The invoice is already saved at this point, so a
      // failure here is reported, never rolled back.
      const timeIds=form.line_items.reduce((a,it)=>Array.isArray(it._time)?a.concat(it._time):a,[]);
      if(timeIds.length&&saved?.id){
        try{await rpcCall('time_mark_invoiced',{p_invoice_id:saved.id,p_entry_ids:timeIds});}
        catch(e2){
          console.warn('[invoice] could not mark time invoiced',e2);
          if(!isMissingTimeRpc(e2))alert('Invoice '+form.invoice_number+' was saved, but the tracked time could not be marked as invoiced:\n\n'+timeErr(e2)+'\n\nThe lines are on the invoice — open Timesheets to check that time.');
        }
      }
      onSaved(saved);
    ...
    <div class="modal-box" style=${{maxWidth:720}}>
      ...
      <div class="modal-body">
        ...
        <div style=${{display:'flex',gap:8,marginTop:4,flexWrap:'wrap'}}>
          <button class="btn-sec" style=${{padding:'6px 12px',fontSize:12}} onClick=${addItem}><i class="ti ti-plus"></i>Add line</button>
          ${timeAvail!==false&&h`<button class="btn-sec" style=${{padding:'6px 12px',fontSize:12}} onClick=${()=>setTimeOpen(true)} title="Bill the time your team tracked for this client"><i class="ti ti-clock-play"></i>Add billable time</button>`}
        </div>
        ${form.line_items.some(it=>Array.isArray(it._time)&&it._time.length)&&h`<div style=${{marginTop:8,fontSize:11.5,color:'var(--t3)'}}>
          <i class="ti ti-clock-check" style=${{fontSize:13,marginRight:4,color:'#15803D'}}></i>
          ${form.line_items.reduce((s,it)=>s+(Array.isArray(it._time)?it._time.length:0),0)} tracked time entries will be marked invoiced when you save.
        </div>`}
        ${linkedTime&&linkedTime.count>0&&h`<div style=${{marginTop:8,display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',fontSize:11.5,color:'var(--t2)'}}>
          <i class="ti ti-clock-check" style=${{fontSize:13,color:'#15803D'}}></i>
          ${fmtMins(linkedTime.minutes)} of tracked time (${linkedTime.count} ${linkedTime.count===1?'entry':'entries'}) is already billed on this invoice.
          <button class="btn-sec" style=${{padding:'3px 9px',fontSize:11}} disabled=${linkBusy} onClick=${releaseTime}>Release</button>
        </div>`}
      ...
      ${timeOpen&&h`<${BillableTimeModal} client=${client} agency=${agency} periodStart=${form.period_start} onClose=${()=>setTimeOpen(false)} onAdd=${addTimeLines}/>`}
    </div>
  </div>`;
}
...
function PaymentHistory({invoice,canDelete,showToast,onChanged}){
  ...
  return h`<div style=${{marginTop:18,paddingTop:14,borderTop:'1px solid var(--bd)'}}>
    ...
    // ── InvoiceViewer ──
function InvoiceViewer({invoice,onClose,onChanged,onEdit,onRenumber,showToast,canEdit=true,clients=[]}){
  ...
  return h`<div class="modal" onClick=${e=>{if(e.target===e.currentTarget)onClose();}}>
    <div class="modal-box inv-modal">
      ...
      <div class="modal-foot" style=${{justifyContent:'space-between',gap:8,flexWrap:'wrap'}}>
        ...
        <div class="inv-actions">
          ${canEdit&&onRenumber&&h`<button class="btn-sec" onClick=${()=>onRenumber(inv)} title="Change this invoice's number"><i class="ti ti-hash"></i>Number</button>`}
...
function TaxReports({invoices,clients,agency,showToast}){
  ...
  const downloadAllPDF=async()=>{
    ...
    setBulkBusy(true);
    try{
      await downloadMergedInvoicePDF(filtered,'Invoices_'+year+'_'+String(month+1).padStart(2,'0')+'.pdf');
      showToast?.(filtered.length+' invoice'+(filtered.length===1?'':'s')+' downloaded');
    }catch(e){
  ...
  return h`<div>
    ...
    // ── Bulk invoice export (shared: Finance ▸ Invoices + Tax & exports) ──
    // One place that knows how to turn any list of invoice rows into the three
    // things a CA asks for: one merged PDF, per-invoice PDFs in a ZIP, and a
    // flat ledger CSV. PDFs render InvoiceTemplate off-screen — never the
    // viewer modal — so the CA gets exactly the document the client saw.
    const _loadLib=(src)=>new Promise((res,rej)=>{const s=document.createElement('script');s.src=src;s.onload=res;s.onerror=()=>rej(new Error('Could not load '+src));document.head.appendChild(s);});
    const ensureHtml2Pdf=async()=>{if(!window.html2pdf)await _loadLib('https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js');return window.html2pdf;};
    const ensureJSZip=async()=>{if(!window.JSZip)await _loadLib('https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js');return window.JSZip;};
    const saveBlob=(blob,filename)=>{const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);};
    const invDateOf=(i)=>i.issued_on||i.period_start||null;
    // Oldest-first everywhere, so the PDF pack reads in the same order as the CSV.
    const sortInvoices=(list)=>[...list].sort((a,b)=>{
      const da=new Date(invDateOf(a)||0),db=new Date(invDateOf(b)||0);
      return da-db||String(a.invoice_number||'').localeCompare(String(b.invoice_number||''));
    });
    const safeName=(s)=>String(s||'').replace(/[^\w\-. ]+/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'').slice(0,60);
    const PDF_OPTS=(filename)=>({
      margin:0,filename,image:{type:'jpeg',quality:0.92},
      // scale 1.6 not 2 — a 50-invoice pack at 2x can exhaust canvas memory.
      html2canvas:{scale:1.6,useCORS:true,backgroundColor:'#ffffff'},
      jsPDF:{unit:'mm',format:'a4',orientation:'portrait'},
      pagebreak:{mode:['css','legacy'],before:'.html2pdf__page-break'},
    });
    // Mounts InvoiceTemplate for `list` off-screen, waits for paint, hands the
    // node to fn(), then always tears the mount down.
    async function withRenderedInvoices(list,fn){
      ...
      try{
        ...
        // createRoot renders concurrently — poll until every .inv-doc has
        // painted rather than guessing a timeout, or html2canvas grabs blanks.
        const ready=await new Promise(res=>{
          let tries=0;
          const poll=()=>{
            if(mount.querySelectorAll('.inv-doc').length>=list.length)return res(true);
            if(++tries>200)return res(false);   // ~10s
            setTimeout(poll,50);
          };
          poll();
        });
    ...
    // Every invoice as one multi-page PDF, one invoice per A4 page.
    async function downloadMergedInvoicePDF(list,filename){
      if(!list.length)return;
      await ensureHtml2Pdf();
      const rows=sortInvoices(list);
      await withRenderedInvoices(rows,mount=>window.html2pdf().from(mount).set(PDF_OPTS(filename)).save());
    }
    // One PDF per invoice, zipped, with the ledger CSV riding along inside.
    async function downloadInvoiceZip(list,filename,ledgerCsv,onProgress){
      if(!list.length)return;
      await ensureHtml2Pdf();
      ...
      for(let idx=0;idx<rows.length;idx++){
        ...
        let name=safeName((inv.type==='credit_note'?'CreditNote-':'Invoice-')+inv.invoice_number+'-'+(inv.client_snapshot?.name||''))+'.pdf';
        if(used[name])name=name.replace(/\.pdf$/,'-'+(++used[name])+'.pdf');else used[name]=1;
        const blob=await withRenderedInvoices([inv],mount=>window.html2pdf().from(mount).set(PDF_OPTS(name)).outputPdf('blob'));
        zip.file(name,blob);
      }
      if(ledgerCsv)zip.file('invoice-ledger.csv','\ufeff'+ledgerCsv);
      const out=await zip.generateAsync({type:'blob'});
      saveBlob(out,filename);
    }
    // The flat register a CA works from: one row per invoice, every GST/TDS/
    // payment field spelled out, credit notes signed negative in the total row.
    function invoiceLedgerCSV(list,clientMap){
      const esc=(s)=>{const v=String(s==null?'':s);return /[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v;};
      const num=(n)=>(Number(n)||0).toFixed(2);
      const dt=(d)=>{if(!d)return'';const x=new Date(d);return String(x.getDate()).padStart(2,'0')+'-'+String(x.getMonth()+1).padStart(2,'0')+'-'+x.getFullYear();};
      const COLS=['Invoice No','Type','Invoice date','Due date','Client','GSTIN','PAN','State','Place of supply','Supply type','HSN/SAC','Period','Subtotal','Discount','Taxable value','GST %','CGST','SGST','IGST','Invoice total','TDS %','TDS deducted','Amount received','Balance due','Status','Payment date','Payment method','Notes'];
      const lines=[COLS.map(esc).join(',')];
      const tot={sub:0,disc:0,tax:0,cgst:0,sgst:0,igst:0,amt:0,tds:0,paid:0,bal:0};
      sortInvoices(list).forEach(i=>{
        const c=i.client_snapshot||clientMap?.[i.client_id]||{};
        const sign=i.type==='credit_note'?-1:1;
        const amount=Number(i.amount)||0,paid=Number(i.amount_paid)||0,tds=Number(i.tds_amount)||0;
        const bal=amount-paid-tds;
        tot.sub+=sign*(Number(i.subtotal)||0);tot.disc+=sign*(Number(i.discount_amount)||0);
        tot.tax+=sign*(Number(i.taxable_value)||0);tot.cgst+=sign*(Number(i.cgst_amount)||0);
        tot.sgst+=sign*(Number(i.sgst_amount)||0);tot.igst+=sign*(Number(i.igst_amount)||0);
        tot.amt+=sign*amount;tot.tds+=sign*tds;tot.paid+=sign*paid;tot.bal+=sign*bal;
        lines.push([
          i.invoice_number,i.type==='credit_note'?'Credit note':'Invoice',
          dt(i.issued_on||i.period_start),dt(i.due_date),
          c.name||clientMap?.[i.client_id]?.name||'',c.gstin||'',c.pan||'',c.state||'',c.place_of_supply||c.state||'',
          i.supply_type||'',i.hsn_sac||'',i.period_label||fmtMonthYear(i.period_start)||'',
          num(i.subtotal),num(i.discount_amount),num(i.taxable_value),num(i.gst_rate),
          num(i.cgst_amount),num(i.sgst_amount),num(i.igst_amount),num(amount),
          num(i.tds_rate),num(tds),num(paid),num(bal),
          i.status||'',dt(i.paid_at),i.payment_method||'',
          (i.notes||'').replace(/\s+/g,' ').trim(),
        ].map(esc).join(','));
      });
      lines.push(['TOTAL','','','',String(list.length)+' documents','','','','','','','',
        num(tot.sub),num(tot.disc),num(tot.tax),'',num(tot.cgst),num(tot.sgst),num(tot.igst),num(tot.amt),
        '',num(tot.tds),num(tot.paid),num(tot.bal),'','','','credit notes subtracted'].map(esc).join(','));
      return lines.join('\n');
    }
...
function BillingApp({clients,currentUser,showToast,canEdit=true}){
  ...
  const[recordingFor,setRecordingFor]=useState(null);
  const[autoTime,setAutoTime]=useState(false);   // open "Add billable time" straight away
  // Filters
  const[statusF,setStatusF]=useState('all');
  const[clientF,setClientF]=useState('all');
  const[searchF,setSearchF]=useState('');
  const[period,setPeriod]=useState(()=>{const d=new Date();return{y:d.getFullYear(),m:'all'};});
  // Bulk export ("everything in one go for the CA"): tick rows, or export
  // whatever the filters currently show when nothing is ticked.
  const[sel,setSel]=useState(()=>new Set());
  const[exportOpen,setExportOpen]=useState(false);
  const[exportBusy,setExportBusy]=useState('');   // '' | 'pdf' | 'zip' | 'csv'
  const[exportProg,setExportProg]=useState('');
  const[renumbering,setRenumbering]=useState(null);   // array of invoices being renumbered
  // Column sort. Default matches what the server sends back (newest first).
  const[sort,setSort]=useState({k:'issued',dir:'desc'});
  const load=()=>{
    setLoading(true);setErr('');
    Promise.all([
      rpcCall('inv_list',{p_limit:2000}),
      rpcCall('agency_settings_get'),
  ...
  // Dates read off the invoice date (issued_on, falling back to the billed
  // period) — that's what a CA files by, and what the tax exports already use.
  // period.m: 'all' | '0'..'11' | 'fy' (Apr–Mar) | 'q1'..'q4' (fiscal quarters).
  const matchPeriod=useCallback((i)=>{
    const raw=invDateOf(i);
    if(!raw)return false;
    const d=new Date(raw);const py=Number(period.y);
    if(period.m==='fy')return d>=new Date(py,3,1)&&d<new Date(py+1,3,1);
    if(/^q[1-4]$/.test(period.m)){const q=Number(period.m[1]);return d>=new Date(py,3+(q-1)*3,1)&&d<new Date(py,3+q*3,1);}
    if(d.getFullYear()!==py)return false;
    if(period.m!=='all'&&d.getMonth()!==Number(period.m))return false;
    return true;
  },[period]);
  ...
  },[invoices,statusF,clientF,matchPeriod,searchF,clientMap]);
  ...
  // Selection is always intersected with what's on screen, so "3 selected"
  // can never quietly export a row you filtered away.
  const selectedRows=useMemo(()=>filtered.filter(i=>sel.has(i.id)),[filtered,sel]);
  const exportRows=selectedRows.length?selectedRows:filtered;
  const allShownSelected=filtered.length>0&&selectedRows.length===filtered.length;
  const toggleRow=(id)=>setSel(prev=>{const n=new Set(prev);n.has(id)?n.delete(id):n.add(id);return n;});
  const toggleAll=()=>setSel(prev=>{
    if(allShownSelected){const n=new Set(prev);filtered.forEach(i=>n.delete(i.id));return n;}
    const n=new Set(prev);filtered.forEach(i=>n.add(i.id));return n;
  });
  const selTotal=useMemo(()=>selectedRows.reduce((s,i)=>s+(i.type==='credit_note'?-1:1)*(Number(i.amount)||0),0),[selectedRows]);
  const MON3=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  // Filename says what's inside — the CA gets "Invoices_All-clients_FY2026-27.zip",
  // not "download (3).zip".
  const exportBase=useMemo(()=>{
    const cl=clientF==='all'?'All-clients':(clientMap[clientF]?.name||'Client').replace(/[^\w]+/g,'-').replace(/^-|-$/g,'');
    const per=period.m==='fy'?('FY'+period.y+'-'+String(Number(period.y)+1).slice(2))
      :/^q[1-4]$/.test(period.m)?(period.m.toUpperCase()+'-FY'+period.y)
      :period.m==='all'?String(period.y)
      :(MON3[Number(period.m)]+'-'+period.y);
    return 'Invoices_'+cl+'_'+per+(selectedRows.length?'_selected':'');
  },[clientF,clientMap,period,selectedRows.length]);
  const periodLabel=period.m==='fy'?('1 Apr '+period.y+' – 31 Mar '+(Number(period.y)+1))
    :/^q[1-4]$/.test(period.m)?(()=>{const q=Number(period.m[1]);const a=new Date(Number(period.y),3+(q-1)*3,1),b=new Date(Number(period.y),3+q*3,0);return MON3[a.getMonth()]+' '+a.getFullYear()+' – '+MON3[b.getMonth()]+' '+b.getFullYear();})()
    :'';
  const runExport=async(kind)=>{
    ...
    if(kind==='csv'){
      // BOM so Excel reads the UTF-8 client names correctly.
      saveBlob(new Blob(['\ufeff'+invoiceLedgerCSV(rows,clientMap)],{type:'text/csv;charset=utf-8'}),exportBase+'.csv');
      showToast(rows.length+' invoice'+(rows.length===1?'':'s')+' exported to CSV');
      return;
    }
    setExportBusy(kind);setExportProg('');
    try{
      if(kind==='pdf')await downloadMergedInvoicePDF(rows,exportBase+'.pdf');
      else await downloadInvoiceZip(rows,exportBase+'.zip',invoiceLedgerCSV(rows,clientMap),(n,t)=>setExportProg(n+'/'+t));
      showToast(rows.length+' invoice'+(rows.length===1?'':'s')+' downloaded');
    }catch(e){
      console.error('[bulk-export] failed',e);
      showToast('Export failed — '+(e?.message||e));
    }finally{setExportBusy('');setExportProg('');}
  };
  // Sorted view of the filtered rows. Invoice numbers compare naturally
  // (A00119 < A00121 < A00129) so the series reads in order when you sort by it.
  const sorted=useMemo(()=>{
    const dir=sort.dir==='asc'?1:-1;
    const txt=(v)=>String(v==null?'':v);
    const val=(i)=>{
      switch(sort.k){
        case'number':return txt(i.invoice_number);
        case'client':return txt(i.client_snapshot?.name||clientMap[i.client_id]?.name);
        case'period':return txt(i.period_start||'');
        case'due':return txt(i.due_date||'');
        case'amount':return Number(i.amount)||0;
        case'status':return txt(i.status);
        default:return txt(invDateOf(i)||'');
      }
    };
    return[...filtered].sort((a,b)=>{
      const x=val(a),y=val(b);
      const c=typeof x==='number'?(x-y):x.localeCompare(y,undefined,{numeric:true,sensitivity:'base'});
      return c*dir||String(a.invoice_number||'').localeCompare(String(b.invoice_number||''),undefined,{numeric:true});
    });
  },[filtered,sort,clientMap]);
  const toggleSort=(k)=>setSort(s0=>s0.k===k?{k,dir:s0.dir==='asc'?'desc':'asc'}:{k,dir:k==='amount'||k==='issued'||k==='due'||k==='period'?'desc':'asc'});
  const EXPORT_ITEMS=[
    ['pdf','ti-file-text','One merged PDF','Every invoice in a single file, one per A4 page'],
    ['zip','ti-file-zip','Separate PDFs (ZIP)','One PDF per invoice + the ledger CSV inside'],
    ['csv','ti-file-spreadsheet','Ledger CSV (Excel)','Every field the CA needs — GST split, TDS, payments'],
  ];
  ...
  // Deep links from Home / the shell. Two window events plus a pending
  // `window.__amsBillingIntent`, because the event is dispatched just before the
  // Finance tab mounts and would otherwise land on nobody.
  const VIEW_ALIASES={gst:'reports',tds:'reports',tax:'reports',taxes:'reports',reports:'reports',exports:'reports',
    aging:'aging',ageing:'aging',income:'income',pnl:'income','p&l':'income',invoices:'invoices',list:'invoices'};
  const applyIntent=useCallback((kind,detail)=>{
    const d=detail||{};
    if(kind==='new'){
      if(!canEdit)return;
      const c=(clients||[]).find(x=>String(x.id)===String(d.clientId));
      setSubView('invoices');setViewing(null);setEditing(null);
      if(c){setEditorClient(c);setAutoTime(!!(d.withTime||d.billTime));}
      else{setAutoTime(false);setPicker(true);}
      return;
    }
    const v=String(d.view||d.tab||'').toLowerCase();
    if(VIEW_ALIASES[v])setSubView(VIEW_ALIASES[v]);
    if(d.invoiceId){
      const inv=invoices.find(i=>String(i.id)===String(d.invoiceId));
      if(inv){setSubView('invoices');setViewing(inv);}
    }
  },[clients,invoices,canEdit]);
  useEffect(()=>{
    const onNew=(e)=>applyIntent('new',e?.detail);
    const onOpen=(e)=>applyIntent('open',e?.detail);
    window.addEventListener('ams-billing-new-invoice',onNew);
    window.addEventListener('ams-billing-open',onOpen);
    const pending=window.__amsBillingIntent;
    if(pending){
      window.__amsBillingIntent=null;
      const p=pending.detail||pending;
      const isNew=/new/i.test(String(pending.type||pending.event||pending.kind||''))||pending.newInvoice===true||(!!p.clientId&&!p.view);
      applyIntent(isNew?'new':'open',p);
    }
    return()=>{window.removeEventListener('ams-billing-new-invoice',onNew);window.removeEventListener('ams-billing-open',onOpen);};
  },[applyIntent]);
  ...
  return h`<div>
    ...
    <div style=${{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',marginBottom:14}}>
      ...
      <select class="fi fi-select" value=${period.m} onChange=${e=>setPeriod(p=>({...p,m:e.target.value}))} style=${{width:168}}>
      ...
      <div style=${{position:'relative'}}>
        <button class="btn-sec" style=${{padding:'7px 14px',fontSize:12}} disabled=${!!exportBusy||filtered.length===0} onClick=${()=>setExportOpen(o=>!o)} title="Download invoices for your CA">
          <i class=${'ti '+(exportBusy?'ti-loader-2 spinner':'ti-download')}></i>${exportBusy?('Building… '+exportProg):'Download'+(selectedRows.length?' ('+selectedRows.length+')':' all')}<i class="ti ti-chevron-down" style=${{fontSize:12,marginLeft:4,opacity:.6}}></i>
        </button>
        ${exportOpen&&h`<div>
          <div style=${{position:'fixed',inset:0,zIndex:60}} onClick=${()=>setExportOpen(false)}></div>
          <div style=${{position:'absolute',right:0,top:'calc(100% + 6px)',zIndex:61,width:300,padding:6,background:'var(--surface)',border:'1px solid var(--bd)',borderRadius:10,boxShadow:'var(--shadow-lg)'}}>
            <div style=${{padding:'8px 10px 6px',fontSize:11,color:'var(--t3)',letterSpacing:'.04em',textTransform:'uppercase',fontWeight:600}}>
              ${exportRows.length} ${selectedRows.length?'selected':'filtered'} invoice${exportRows.length===1?'':'s'}
            </div>
            ${EXPORT_ITEMS.map(([k,ic,t1,t2])=>h`<button key=${k} class="dl-item" onClick=${()=>runExport(k)}>
              <i class=${'ti '+ic}></i>
              <span><span class="dl-item-t">${t1}</span><span class="dl-item-s">${t2}</span></span>
            </button>`)}
            <div style=${{padding:'7px 10px 4px',fontSize:11,color:'var(--t3)',lineHeight:1.5,borderTop:'1px solid var(--bd)',marginTop:4}}>Tick rows to export just those. Big packs take a minute — keep this tab open.</div>
          </div>
        </div>`}
      </div>
    ...
    ${selectedRows.length>0&&h`<div style=${{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',padding:'9px 12px',marginBottom:12,borderRadius:10,background:'rgba(255,0,238,.06)',border:'1px solid rgba(255,0,238,.25)'}}>
      <div style=${{fontSize:12.5,color:'var(--t1)',fontWeight:600}}>${selectedRows.length} selected<span style=${{fontWeight:400,color:'var(--t2)'}}> · ₹${fmtINR(selTotal,false)}</span></div>
      <div style=${{flex:1}}></div>
      ${EXPORT_ITEMS.map(([k,ic,t1])=>h`<button key=${k} class="btn-sec" style=${{padding:'6px 11px',fontSize:11.5}} disabled=${!!exportBusy} onClick=${()=>runExport(k)}><i class=${'ti '+(exportBusy===k?'ti-loader-2 spinner':ic)}></i>${t1}</button>`)}
      ${canEdit&&h`<button class="btn-sec" style=${{padding:'6px 11px',fontSize:11.5}} onClick=${()=>setRenumbering(selectedRows)}><i class="ti ti-list-numbers"></i>Renumber</button>`}
      <button class="btn-sec" style=${{padding:'6px 11px',fontSize:11.5,color:'var(--t2)'}} onClick=${()=>setSel(new Set())}>Clear</button>
    </div>`}
    ...
    ${loading?h`<div style=${{padding:24,textAlign:'center',color:'var(--t3)',fontSize:13}}>Loading invoices…</div>`
      ...
      :h`<div style=${{border:'1px solid var(--bd)',borderRadius:10,overflow:'hidden',background:'var(--surface)'}}>
        <table style=${{width:'100%',borderCollapse:'collapse',fontSize:13}}>
          <thead><tr style=${{background:'var(--bg2)',color:'var(--t3)',fontSize:11,letterSpacing:'.05em',textTransform:'uppercase',fontWeight:500}}>
            <th style=${{padding:'10px 4px 10px 12px',width:30}}><input type="checkbox" title="Select every invoice shown" checked=${allShownSelected} onChange=${toggleAll} style=${{cursor:'pointer',accentColor:'#ff00ee',width:15,height:15}}/></th>
            <${SortTh} k="number" label="Invoice" sort=${sort} onSort=${toggleSort}/>
            <${SortTh} k="client" label="Client" sort=${sort} onSort=${toggleSort}/>
            <${SortTh} k="period" label="Period" sort=${sort} onSort=${toggleSort}/>
            <${SortTh} k="issued" label="Issued" sort=${sort} onSort=${toggleSort}/>
            <${SortTh} k="due" label="Due" sort=${sort} onSort=${toggleSort}/>
            <${SortTh} k="amount" label="Amount" align="right" sort=${sort} onSort=${toggleSort}/>
            <${SortTh} k="status" label="Status" sort=${sort} onSort=${toggleSort}/>
            <th style=${{padding:'10px 12px',width:104,textAlign:'right'}}></th>
          </tr></thead>
          <tbody>
            ${sorted.map(i=>{const c=clientMap[i.client_id];const st=STATUS_STYLE[i.status]||STATUS_STYLE.due;const paid=Number(i.amount_paid)||0;const total=Number(i.amount)||0;const on=sel.has(i.id);return h`<tr key=${i.id} onClick=${()=>setViewing(i)} style=${{cursor:'pointer',borderTop:'1px solid var(--bd)',background:on?'rgba(255,0,238,.05)':'transparent'}}>
              <td style=${{padding:'10px 4px 10px 12px'}} onClick=${e=>e.stopPropagation()}><input type="checkbox" checked=${on} onChange=${()=>toggleRow(i.id)} style=${{cursor:'pointer',accentColor:'#ff00ee',width:15,height:15}}/></td>
              ...
              <td style=${{padding:'10px 12px',whiteSpace:'nowrap'}} onClick=${e=>e.stopPropagation()}>
                ${canEdit&&h`<button class="icon-btn" style=${{padding:5,marginRight:4}} onClick=${()=>setRenumbering([i])} title="Change invoice number"><i class="ti ti-hash" style=${{color:'var(--t2)',fontSize:15}}></i></button>`}
    ...
    <div style=${{marginTop:12,fontSize:11.5,color:'var(--t3)'}}>${filtered.length} of ${invoices.length} invoices${periodLabel?' · '+periodLabel:''} · click a column header to sort · tick rows and hit <strong>Download</strong> for the CA pack, <strong>Renumber</strong> to fix the series, or click any row to ${canEdit?'view, print, or record a payment':'view, print, or download'}</div>
    </div>`}

    ${picker&&h`<${ClientPickerModal} clients=${clients} onClose=${()=>setPicker(false)} onPick=${openNewInvoice}/>`}
    ${editorClient&&h`<${InvoiceEditor} client=${editorClient} agency=${agency} existing=${editing} autoTime=${autoTime} onClose=${()=>{setEditorClient(null);setEditing(null);setAutoTime(false);}} onSaved=${(saved)=>{setAutoTime(false);onSaved(saved);}}/>`}
    ${viewing&&!editorClient&&h`<${InvoiceViewer} invoice=${viewing} clients=${clients} onClose=${()=>setViewing(null)} onChanged=${(u)=>{setViewing(u);load();}} onEdit=${openEdit} onRenumber=${(inv)=>{setViewing(null);setRenumbering([inv]);}} showToast=${showToast} canEdit=${canEdit}/>`}
    ${renumbering&&h`<${RenumberModal} targets=${renumbering} allInvoices=${invoices} onClose=${()=>setRenumbering(null)} onDone=${()=>{setRenumbering(null);setSel(new Set());load();}} showToast=${showToast}/>`}
    ...
    // ── SortTh ── sortable header cell for the invoice table
function SortTh({k,label,align,sort,onSort}){
  const on=sort.k===k;
  return h`<th style=${{padding:'10px 12px',textAlign:align||'left',cursor:'pointer',userSelect:'none',whiteSpace:'nowrap'}} onClick=${()=>onSort(k)} title=${'Sort by '+label}>
    ${label}<i class=${'ti '+(on?(sort.dir==='asc'?'ti-arrow-up':'ti-arrow-down'):'ti-arrows-sort')} style=${{fontSize:12,marginLeft:4,opacity:on?.9:.35,verticalAlign:'-1px'}}></i>
  </th>`;
}

    // ── RenumberModal ──
// Fixes invoice numbering after the fact. GST expects one consecutive series
// per financial year, but numbers drift — imports land out of order, drafts get
// abandoned, invoices are raised on the wrong day. This edits the number on ANY
// invoice (paid ones included — the number is metadata, not money) either one at
// a time or as a whole run.
//
// Two safety rails: nothing applies until the whole preview is conflict-free
// (checked agency-wide, not just against the DB's per-client unique index), and
// a batch where two invoices swap numbers is written in two passes through a
// temporary name so the unique index can never trip mid-run.
function RenumberModal({targets,allInvoices,onClose,onDone,showToast}){
  const seed=useMemo(()=>{
    const rows=[...targets].sort((a,b)=>{
      const da=new Date(invDateOf(a)||0),db=new Date(invDateOf(b)||0);
      return da-db||String(a.invoice_number||'').localeCompare(String(b.invoice_number||''),undefined,{numeric:true});
    });
    return rows.map(i=>({id:i.id,inv:i,next:i.invoice_number||''}));
  },[targets]);
  const[rows,setRows]=useState(seed);
  const[busy,setBusy]=useState('');
  const[err,setErr]=useState('');
  // Series generator, prefilled from the batch's own numbers (A00119 → "A" + 5 digits).
  const parsed=(()=>{const m=/^(\D*)(\d+)$/.exec(String(seed[0]?.inv?.invoice_number||''));return m?{prefix:m[1],pad:m[2].length,start:Number(m[2])}:{prefix:'A',pad:5,start:1};})();
  const[prefix,setPrefix]=useState(parsed.prefix);
  const[start,setStart]=useState(parsed.start);
  const[pad,setPad]=useState(parsed.pad);
  const[order,setOrder]=useState('date');   // 'date' | 'current'
  const setNext=(id,v)=>setRows(rs=>rs.map(r=>r.id===id?{...r,next:v}:r));
  const fillSeries=()=>{
    const ordered=[...rows].sort((a,b)=>{
      if(order==='current')return String(a.inv.invoice_number||'').localeCompare(String(b.inv.invoice_number||''),undefined,{numeric:true});
      const da=new Date(invDateOf(a.inv)||0),db=new Date(invDateOf(b.inv)||0);
      return da-db||String(a.inv.invoice_number||'').localeCompare(String(b.inv.invoice_number||''),undefined,{numeric:true});
    });
    const map={};ordered.forEach((r,idx)=>{map[r.id]=prefix+String(Number(start)+idx).padStart(Number(pad)||1,'0');});
    setRows(rs=>rs.map(r=>({...r,next:map[r.id]||r.next})));
  };
  // Conflicts: blank, duplicated inside the batch, or already taken by an
  // invoice outside it. Agency-wide on purpose — a CA reading the register
  // shouldn't see the same number twice under two different clients.
  const batchIds=useMemo(()=>new Set(rows.map(r=>r.id)),[rows]);
  const outsideMap=useMemo(()=>{
    const m={};(allInvoices||[]).forEach(i=>{if(!batchIds.has(i.id)&&i.invoice_number)m[String(i.invoice_number).trim().toLowerCase()]=i;});
    return m;
  },[allInvoices,batchIds]);
  const conflicts=useMemo(()=>{
    const seen={},out={};
    rows.forEach(r=>{const k=String(r.next||'').trim().toLowerCase();if(k)seen[k]=(seen[k]||0)+1;});
    rows.forEach(r=>{
      const v=String(r.next||'').trim();const k=v.toLowerCase();
      if(!v)out[r.id]='Required';
      else if(seen[k]>1)out[r.id]='Repeated in this list';
      else if(outsideMap[k])out[r.id]='Already used'+(outsideMap[k].client_snapshot?.name?' by '+outsideMap[k].client_snapshot.name:'');
    });
    return out;
  },[rows,outsideMap]);
  const changed=rows.filter(r=>String(r.next||'').trim()!==String(r.inv.invoice_number||''));
  const blocked=Object.keys(conflicts).length>0;
  const apply=async()=>{
    if(blocked||!changed.length)return;
    setErr('');setBusy('1');
    try{
      // Pass 1 — only rows whose target is currently held by ANOTHER row in the
      // batch (a swap or a shift-by-one run) need parking under a temp name.
      const currentByKey={};rows.forEach(r=>{currentByKey[String(r.inv.invoice_number||'').trim().toLowerCase()]=r.id;});
      const needsTemp=changed.filter(r=>{const holder=currentByKey[String(r.next).trim().toLowerCase()];return holder&&holder!==r.id;});
      for(let n=0;n<needsTemp.length;n++){
        const r=needsTemp[n];
        setBusy('parking '+(n+1)+'/'+needsTemp.length);
        await rpcCall('inv_update',{p_id:r.id,p_data:{invoice_number:'TMP-'+String(r.id).slice(0,8)+'-'+n}});
      }
      for(let n=0;n<changed.length;n++){
        const r=changed[n];
        setBusy((n+1)+'/'+changed.length);
        await rpcCall('inv_update',{p_id:r.id,p_data:{invoice_number:String(r.next).trim()}});
        logActivity({action:'update',table_name:'invoices',record_id:String(r.id),record_label:(r.inv.invoice_number||'—')+' → '+String(r.next).trim(),changes:{invoice_number:{old:r.inv.invoice_number,new:String(r.next).trim()}}});
      }
      showToast?.(changed.length===1?'Invoice number updated':changed.length+' invoices renumbered');
      onDone?.();
    }catch(e){
      console.error('[renumber] failed',e);
      setErr('Stopped partway: '+(e?.message||e)+' — reopen to see where it got to.');
      setBusy('');
    }
  };
  const single=rows.length===1;
  return h`<div class="modal" onClick=${e=>{if(e.target===e.currentTarget&&!busy)onClose();}}>
    <div class="modal-box" style=${{maxWidth:640,display:'flex',flexDirection:'column',maxHeight:'88vh'}}>
      <div class="modal-head">
        <div>
          <div style=${{fontSize:16,fontWeight:600}}>${single?'Change invoice number':'Renumber '+rows.length+' invoices'}</div>
          <div style=${{fontSize:12,color:'var(--t3)',marginTop:2}}>${single?'Works on paid invoices too — the number is a label, not the money.':'Put the series back in order. Nothing is written until every row is conflict-free.'}</div>
        </div>
        <button class="icon-btn" onClick=${onClose} disabled=${!!busy}><i class="ti ti-x"></i></button>
      </div>
      <div class="modal-body" style=${{flex:1,overflowY:'auto'}}>
        ${!single&&h`<div style=${{display:'flex',gap:8,alignItems:'flex-end',flexWrap:'wrap',padding:'12px 14px',background:'var(--bg2)',borderRadius:10,marginBottom:14}}>
          <div style=${{width:74}}><div class="fi-lbl">Prefix</div><input class="fi" type="text" value=${prefix} onInput=${e=>setPrefix(e.target.value)} placeholder="A"/></div>
          <div style=${{width:96}}><div class="fi-lbl">Start at</div><input class="fi" type="number" min="0" value=${start} onInput=${e=>setStart(e.target.value)}/></div>
          <div style=${{width:78}}><div class="fi-lbl">Digits</div><input class="fi" type="number" min="1" max="10" value=${pad} onInput=${e=>setPad(e.target.value)}/></div>
          <div style=${{width:150}}><div class="fi-lbl">Order by</div>
            <select class="fi fi-select" value=${order} onChange=${e=>setOrder(e.target.value)}>
              <option value="date">Invoice date</option>
              <option value="current">Current number</option>
            </select>
          </div>
          <button class="btn-sec" style=${{padding:'8px 14px',fontSize:12}} onClick=${fillSeries}><i class="ti ti-list-numbers"></i>Fill sequence</button>
        </div>`}
        <table style=${{width:'100%',borderCollapse:'collapse',fontSize:12.5}}>
          <thead><tr style=${{color:'var(--t3)',fontSize:10.5,textTransform:'uppercase',letterSpacing:'.05em'}}>
            <th style=${{textAlign:'left',padding:'6px 8px'}}>Date</th>
            <th style=${{textAlign:'left',padding:'6px 8px'}}>Client</th>
            <th style=${{textAlign:'left',padding:'6px 8px'}}>Current</th>
            <th style=${{textAlign:'left',padding:'6px 8px',width:190}}>New number</th>
          </tr></thead>
          <tbody>
            ${rows.map(r=>h`<tr key=${r.id} style=${{borderTop:'1px solid var(--bd)'}}>
              <td style=${{padding:'6px 8px',color:'var(--t2)',whiteSpace:'nowrap'}}>${fmt(invDateOf(r.inv))}</td>
              <td style=${{padding:'6px 8px',color:'var(--t1)'}}>${r.inv.client_snapshot?.name||'—'}</td>
              <td style=${{padding:'6px 8px',color:'var(--t3)'}}>${r.inv.invoice_number||'—'}</td>
              <td style=${{padding:'6px 8px'}}>
                <input class="fi" type="text" value=${r.next} onInput=${e=>setNext(r.id,e.target.value)} style=${{padding:'6px 9px',fontSize:12.5,borderColor:conflicts[r.id]?'#DC2626':undefined}}/>
                ${conflicts[r.id]&&h`<div style=${{fontSize:10.5,color:'#DC2626',marginTop:3}}>${conflicts[r.id]}</div>`}
              </td>
            </tr>`)}
          </tbody>
        </table>
        ${err&&h`<div class="err" style=${{marginTop:12}}><i class="ti ti-alert-circle"></i>${err}</div>`}
        <div style=${{marginTop:14,fontSize:11.5,color:'var(--t3)',lineHeight:1.55}}>
          Invoices already emailed or downloaded keep the old number in the client's copy — resend from the ✉ composer if the new number matters to them.
        </div>
      </div>
      <div class="modal-foot" style=${{justifyContent:'space-between'}}>
        <div style=${{fontSize:12,color:blocked?'#DC2626':'var(--t3)'}}>${blocked?'Fix the flagged rows to continue':changed.length?changed.length+' number'+(changed.length===1?'':'s')+' will change':'No changes yet'}</div>
        <div style=${{display:'flex',gap:8}}>
          <button class="btn-sec" onClick=${onClose} disabled=${!!busy}>Cancel</button>
          <button class="btn-pri" onClick=${apply} disabled=${blocked||!changed.length||!!busy}>${busy?h`<i class="ti ti-loader-2 spinner"></i>Saving ${busy==='1'?'':busy}`:'Apply'}</button>
        </div>
      </div>
    </div>
  </div>`;
