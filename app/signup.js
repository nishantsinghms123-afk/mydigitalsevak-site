...
// Signup captures a MANDATORY contact number (migration 086) — trials that arrive
// from SEO are unreachable without one; it lands on orgs.contact_phone and shows
// in the platform Tenants console.
...
(function(){
  function buildSignup(deps){
    const { React, h, useState, useEffect, supabase, SB_URL, SB_KEY, APP_VERSION,
            googleSignIn, googleLoginEnabled, GoogleG, setSessionToken, normPhone } = deps;
    ...
    function SignupPage({ onBackToLogin, onGoogleLogin }){
      ...
      const googleSignup=async()=>{
        const o=org.trim(), ph=normPhone(phone);
        if(!o){ setErr('Enter your agency name first, then continue with Google.'); return; }
        if(!ph){ setErr('Add a valid contact number first, then continue with Google.'); return; }
        setGLoading(true); setErr('');
        try{
          const res=await googleSignIn('signup',o,ph);
          if(res.session_token&&setSessionToken)setSessionToken(res.session_token);
          onGoogleLogin&&onGoogleLogin(res);
        }catch(e){
          const code=String(e.code||e.message||'');
          setErr(code==='phone_required'?'Add a valid contact number, then continue with Google.'
                :code==='org_name_required'?'Enter your agency name first, then continue with Google.'
                :(e.message||'Google sign-up failed. Try again.'));
          setGLoading(false);
        }
      };
      ...
      const submit=async()=>{
        const o=org.trim(), e=email.trim().toLowerCase(), ph=normPhone(phone);
        if(!o){ setErr('Enter your agency name.'); return; }
        if(!e || e.indexOf('@')<1){ setErr('Enter a valid work email.'); return; }
        if(!ph){ setErr('Enter a valid contact number — we use it to help you set up.'); return; }
        setLoading(true); setErr('');
        try{
          const res=await fetch(`${SB_URL}/functions/v1/tenant-signup`,{
            method:'POST',
            headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${SB_KEY}`, apikey:SB_KEY },
            body:JSON.stringify({ org_name:o, admin_name:name.trim(), admin_email:e, phone:ph, plan }),
          });
          const data=await res.json().catch(()=>({}));
          if(!res.ok){
            setErr(data.message || (data.error==='email_taken'?'That email is already registered — try signing in.':data.error==='invalid_phone'?'Enter a valid contact number.':'Could not create your account. Try again.'));
      ...
      return h`<div class="auth-stage"><div class="auth-shell">
        <div class="auth-form-side" style=${{overflowY:'auto'}}>
          ...
          <div class="auth-form-inner">
            ...
            <div class="auth-field">
              <label class="auth-label" for="su-phone">Contact number</label>
              <input id="su-phone" class="auth-input" type="tel" inputMode="tel" placeholder="+91 98765 43210" value=${phone} onInput=${e=>setPhone(e.target.value)} onKeyDown=${e=>e.key==='Enter'&&submit()} autoComplete="tel"/>
              <p style=${{fontSize:11.5,color:'var(--t3)',margin:'6px 0 0',lineHeight:1.5}}>
                ${phone.trim()&&!normPhone(phone)
                  ? h`<span style=${{color:'var(--red)'}}><i class="ti ti-alert-circle"></i> That doesn’t look like a valid number.</span>`
                  : 'So we can help you set up your workspace. WhatsApp works best.'}
              </p>
            </div>
            ${err&&h`<div class="auth-error"><i class="ti ti-alert-circle"></i>${err}</div>`}
            <button class="auth-submit" onClick=${submit} disabled=${loading||!org.trim()||!email.trim()||!normPhone(phone)}>
