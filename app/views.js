 ...
 * Gantt / Workload) + saved views, toolbar, filter builder, bulk bar.
 *
 *   window.AMS_VIEWS.buildViews(deps) → { TaskViews, QuickAddRow }
 *
 * Contract: docs/clickup-parity-contract.md §6.2 (shapes §4, filters §5,
 * tasks.js atoms §6.1). All data comes from tasks.js via deps (useTasks,
 * useTaskStore, TaskAPI, pickers). No polling here — useTasks owns refresh.
...
(function () {
  function buildViews(deps) {
    const { React, h, useState, useEffect, useRef, useCallback, useMemo,
            ...
            Popover, Menu, StatusPicker, StatusIcon, PriorityFlag, PriorityPicker,
            AssigneeStack, AssigneePicker, DueChip, DatePicker, TagChips, TagPicker,
            ClientBadge, TypeIcon, MemberAvatar, EmptyState } = deps;
    ...
    if (!document.getElementById('ams-views-styles')) {
      ...
      @media(max-width:640px){.cv-search input{width:100px}.cv-hide-sm{display:none}.cv-drawer{width:220px;flex-basis:220px}}
      @media(prefers-reduced-motion:reduce){.cv-root *,.cv-side *,.cv-bulk *{transition:none!important;animation:none!important}}
      `;
    ...
    const VIEW_TYPES = [
      ...
      { key: 'calendar', label: 'Calendar', icon: 'ti-calendar', desc: 'Plan tasks by due date' },
      { key: 'table', label: 'Table', icon: 'ti-table', desc: 'Spreadsheet grid of fields' },
      { key: 'gantt', label: 'Gantt', icon: 'ti-timeline', desc: 'Timeline of start and due dates' },
      { key: 'workload', label: 'Workload', icon: 'ti-chart-bar', desc: 'Team capacity by day' },
    ];
    ...
    const defaultConfigFor = (type) => {
      ...
      if (type === 'gantt' || type === 'calendar' || type === 'workload') c.group_by = 'none';
      return c;
    };
    ...
    function BulkBar({ ids, vc, onClear }) {
      const [busy, setBusy] = useState(false);
      const stPop = usePop(), asPop = usePop(), prPop = usePop(), duePop = usePop(), mvPop = usePop();
    ...
    function TaskRowItem({ row, vc, depth = 0, cols, tpl }) {
      ...
      return h`<${Fragment}>
        <div class=${'cv-row' + (selected ? ' sel' : '')} style=${{ gridTemplateColumns: tpl }} role="row" aria-selected=${selected}>
          ...
          <div class="cv-cell cv-name" style=${{ paddingLeft: 4 + depth * 22 }}>
            ${hasSubs
            ...
            ${showClient && h`<span style=${{ display: 'inline-flex', flexShrink: 0 }}><${ClientBadge} clientId=${row.client_id}/></span>`}
            ${vc.cfg.show_locations && row.list_name && h`<span class="cv-loc">${(row.client_name ? row.client_name + ' › ' : '') + row.list_name}</span>`}
          </div>
    ...
    function BoardCard({ row, vc, g, groups, dragging, dropBefore, onDragStart, onDragEnd, onDragOverCard }) {
      ...
      const showClient = !vc.singleClient && row.client_id;
      return h`<div class=${'cv-card' + (dragging ? ' dragging' : '') + (dropBefore ? ' dropbefore' : '') + (vc.selected.has(row.id) ? ' sel' : '')}
          draggable="true" tabIndex=${0} role="listitem" aria-label=${row.title}
    ...
    function BoardMode({ vc, groups }) {
      ...
      return h`<div class="cv-board" role="list" aria-label="Board">
        ${groups.map(g => {
          ...
          return h`<div key=${g.key} class=${'cv-bcol' + (isOver && drag.fromKey !== g.key ? ' drop' : '')}
            ...
            <div class="cv-bhd">
              ${pill}<span class="cv-gcount">${g.rows.length}</span><span style=${{ flex: 1 }}></span>
    ...
    function CalendarMode({ vc, rows }) {
      ...
      const mode = vc.cfg.calendar_mode === 'week' ? 'week' : 'month';
      ...
      const step = (n) => setCursor(c => mode === 'week' ? addDays(c, 7 * n) : new Date(c.getFullYear(), c.getMonth() + n, 1));
      ...
      return h`<div class="cv-cal">
        ...
        <div class="cv-calwrap">
          ${mode === 'month' ? h`<div class="cv-month" role="grid" aria-label=${title}>
              ${DOW.map(d => h`<div key=${d} class="cv-dow" role="columnheader">${d}</div>`)}
              ${days.map(day => {
          ...
          : h`<div class="cv-week" role="grid" aria-label=${title}>
              ${days.map((day, i) => {
                ...
                return h`<div key=${k} class=${'cv-wcol' + (sameDay(day, today) ? ' today' : '') + (overDay === k ? ' drop' : '')} role="gridcell" ...${dayDrop(day)}>
                  <div class="cv-whd"><span>${DOW[i]}</span><b>${day.getDate()}</b><span class="cv-sp"></span>
        ...
        <table class="cv-tbl" style=${{ width: totalW }} role="grid" aria-rowcount=${rows.length}>
          ...
          <tbody>
            ${rows.slice(0, cap).map((r, i) => {
              ...
              return h`<tr key=${r.id} aria-selected=${sel}>
    ...
    function GanttMode({ vc, rows }) {
      ...
      const [drag, setDrag] = useState(null);   // { id, kind, x0, dd }
      const [cap, setCap] = useState(GROUP_CAP);
      const today = sod(new Date());
      ...
      const ticks = useMemo(() => {
        ...
        for (let i = 0; i < range.days; i++) {
          ...
          if (zoom === 'month') {
            ...
            if (zoom === 'day') small.push({ x: i * px, label: d.getDate() + ' ' + 'MTWTFSS'[(d.getDay() + 6) % 7], w: px });
            else if (d.getDay() === 1) small.push({ x: i * px, label: String(d.getDate()), w: 7 * px });
          }
        }
      ...
      const onBarMove = (e) => {
        ...
        if (dd !== drag.dd || (!drag.moved && Math.abs(dx) > 3)) setDrag(d => ({ ...d, dd, moved: d.moved || Math.abs(dx) > 3 }));
      };
      const onBarUp = (e, row) => {
        if (!drag) return;
        e.stopPropagation();
      ...
      const barFor = (r) => {
        const off = drag && drag.id === r.id ? drag.dd : 0;
        const endOff = drag && drag.id === r.id && drag.kind === 'end' ? drag.dd : off;
        const startOff = drag && drag.id === r.id && drag.kind === 'end' ? 0 : off;
        const color = r.status_color || '#87909e';
        const handlers = { onPointerMove: onBarMove, onPointerUp: (e) => onBarUp(e, r), onPointerCancel: () => setDrag(null) };
        if (r.start_at && r.due_at) {
          ...
          return h`<div class=${'cv-bar' + (drag && drag.id === r.id ? ' dragging' : '')} style=${{ left, width: w, background: color }}
              title=${r.title + ' · ' + fmtShort(r.start_at) + ' → ' + fmtShort(r.due_at)} role="button" tabIndex=${0} aria-label=${r.title}
              onPointerDown=${(e) => onBarDown(e, r, 'move')} ...${handlers} onKeyDown=${(e) => e.key === 'Enter' && openTask(r.id)}>
            <span style=${{ overflow: 'hidden', textOverflow: 'ellipsis' }}>${w > 60 ? r.title : ''}</span>
            <span class="cv-hdl" aria-label="Drag to change due date" onPointerDown=${(e) => onBarDown(e, r, 'end')} ...${handlers}></span>
          </div>`;
        }
        const one = sod(r.due_at || r.start_at);
        const d = addDays(one, off);
        if (!r.start_at) {
          return h`<div class="cv-diamond" style=${{ left: xOf(d) + px / 2 - 8, background: color }} title=${r.title + ' · due ' + fmtShort(r.due_at)}
            role="button" tabIndex=${0} aria-label=${r.title} onPointerDown=${(e) => onBarDown(e, r, 'move')} ...${handlers} onKeyDown=${(e) => e.key === 'Enter' && openTask(r.id)}></div>`;
        }
        return h`<div class="cv-bar" style=${{ left: xOf(d), width: px, background: color }} title=${r.title} role="button" tabIndex=${0} aria-label=${r.title}
          onPointerDown=${(e) => onBarDown(e, r, 'move')} ...${handlers}></div>`;
      };
      ...
      const bodyH = (Math.min(dated.length, cap)) * 34;
      ...
      return h`<div class="cv-gantt" ref=${rootRef} style=${{ width: GANTT_NAME_W + width }}>
        ...
        : h`<${Fragment}>
          ...
          <div style=${{ position: 'relative' }}>
            ...
            ${dated.slice(0, cap).map(r => h`<div key=${r.id} class="cv-grow">
              ${nameCell(r)}
          ...
          ${undated.length > 0 && h`<${Fragment}>
            ...
            ${undated.slice(0, GROUP_CAP).map(r => h`<div key=${r.id} class="cv-grow">
              ${nameCell(r)}
    ...
    function WorkloadMode({ vc, rows }) {
      ...
      const grid = useMemo(() => {
        const g = {};   // memberId → { days:[{rows,mins}], total, mins }
        const cell = (mid) => g[mid] || (g[mid] = { days: Array.from({ length: 7 }, () => ({ rows: [], mins: 0 })), total: 0, mins: 0 });
        rows.forEach(r => {
      ...
      return h`<div class="cv-wl">
        <div class="cv-calbar" style=${{ padding: '8px 0' }}>
          ...
          <span class="cv-sp"></span>
          <span class="cv-loc"><span class="cv-tone-ok">●</span> under 80% · <span class="cv-tone-near">●</span> 80–100% · <span class="cv-tone-over">●</span> over capacity</span>
        </div>
        <div class="cv-wlgrid" role="grid" aria-label=${'Workload ' + label}>
          <div class="cv-wlhd cv-wlmem" role="columnheader">Member</div>
          ${days.map((d, i) => h`<div key=${i} role="columnheader" class=${'cv-wlhd' + (sameDay(d, today) ? ' today' : '')} style=${{ justifyContent: 'center' }}>${DOW[i]} ${d.getDate()}</div>`)}
          <div class="cv-wlhd" role="columnheader">Week total</div>
          ${members.map(m => {
            const g = grid[m.id] || { days: Array.from({ length: 7 }, () => ({ rows: [], mins: 0 })), total: 0, mins: 0 };
            const cap = Number(m.capacity) || 0;
            const tone = loadTone(g.total, cap);
            ...
            return h`<${Fragment} key=${m.id}>
              <div class="cv-wlmem" role="rowheader">
                ...
                <div style=${{ minWidth: 0 }}>
                  ...
                  <div class="cv-loc">${cap ? 'Capacity ' + cap + '/wk' : 'No capacity set'}</div>
                </div>
              </div>
              ${g.days.map((c, di) => h`<div key=${di} class="cv-wlcell" role="gridcell">
                <button type="button" class=${c.rows.length ? 'has' : ''} disabled=${!c.rows.length}
                  aria-label=${m.name + ', ' + DOW[di] + ': ' + c.rows.length + ' tasks'}
                  ...
                  ${c.mins > 0 && h`<small>${fmtMinutes(c.mins)}</small>`}
                </button>
              </div>`)}
              <div class="cv-wltot" role="gridcell">
                <div style=${{ display: 'flex', justifyContent: 'space-between', gap: 6, fontSize: 12.5 }}>
                  <b class=${tone ? 'cv-tone-' + tone : ''}>${g.total}${cap ? ' / ' + cap : ''}</b>
                  ${g.mins > 0 && h`<span class="cv-loc">${fmtMinutes(g.mins)}</span>`}
                </div>
                ${cap > 0 && h`<div class="cv-meter" role="meter" aria-valuemin=${0} aria-valuemax=${cap} aria-valuenow=${g.total}><i style=${{ width: pct + '%', background: 'var(--cv-' + tone + ')' }}></i></div>`}
              </div>
            <//>`;
          })}
        </div>
        ${cellPop && h`<${Popover} anchor=${cellPop.anchor} open=${true} onClose=${() => setCellPop(null)} width=${280}>
          <div class="cv-pop">
            <div class="cv-pop-h">${DOW[cellPop.di]} ${days[cellPop.di].getDate()} · ${popRows.length} task${popRows.length === 1 ? '' : 's'}</div>
            <div style=${{ maxHeight: 300, overflowY: 'auto' }}>
              ${popRows.map(r => h`<button type="button" key=${r.id} class="cv-mi" onClick=${() => { setCellPop(null); openTask(r.id); }}>
    ...
    function ColumnsChooser({ vc }) {
      const cols = vc.columns;
      const all = Object.keys(COLS).concat((vc.availableFields || []).map(f => 'cf:' + f.id));
    ...
    function AddViewPopover({ onCreate }) {
      ...
      return h`<div class="cv-pop" style=${{ padding: 8 }}>
        <div class="cv-pop-h">Add a view</div>
        <div class="cv-catalog">
          ${VIEW_TYPES.map(v => h`<button type="button" key=${v.key} class="cv-cat" onClick=${() => onCreate(v.key, { is_private: priv, is_pinned: pin })}>
            <i class=${'ti ' + v.icon}></i><div><b>${v.label}</b><span>${v.desc}</span></div>
          </button>`)}
        </div>
    ...
    function CustomizePanel({ vc, tab, meId, onClose, onRename, onFlag, onSaveNew, onDelete, onSave, dirty }) {
      const view = tab.view;
      ...
      return h`<${Fragment}>
        ...
        <aside class="cv-side" role="dialog" aria-modal="true" aria-label="Customize view">
          <div class="cv-side-hd"><i class=${'ti ' + (VT[tab.type] || VT.list).icon}></i><span style=${{ flex: 1 }}>Customize view</span>
          ...
          <div class="cv-side-bd">
            ...
            <div style=${{ display: 'flex', gap: 6, padding: '0 8px 10px' }}>
              <input class="cv-in" style=${{ flex: 1 }} aria-label="View name" value=${name} disabled=${!view || locked} onChange=${e => setName(e.target.value)}
                onKeyDown=${e => { if (e.key === 'Enter' && name.trim()) onRename(name.trim()); }}/>
              <button type="button" class="cv-btn" disabled=${!view || locked || !name.trim() || name.trim() === tab.name} onClick=${() => onRename(name.trim())}>Rename</button>
            </div>
            ${!view && h`<div class="cv-loc" style=${{ padding: '0 8px 10px' }}>This is a built-in view. Save it as a new view to rename, pin or share it.</div>`}
          ...
          <div class="cv-side-ft">
            ...
            <span class="cv-sp"></span>
            ${view && h`<button type="button" class="cv-btn cv-danger" disabled=${locked} onClick=${onDelete}><i class="ti ti-trash"></i>Delete view</button>`}
          </div>
        </aside>
      <//>`;
    }
    ...
    function TaskViews({ scope, currentUser, showToast, initialView, compact }) {
      ...
      // ---- tabs: built-ins + saved views for this scope ----
      ...
      const tabs = useMemo(() => [
        ...VIEW_TYPES.map(t => ({ key: 'builtin:' + t.key, type: t.key, name: t.label, icon: t.icon, builtin: true, config: defaultConfigFor(t.key) })),
        ...savedViews.map(v => ({ key: v.id, type: VT[v.view_type] ? v.view_type : 'list', name: v.name, icon: (VT[v.view_type] || VT.list).icon, view: v, config: normConfig(v.view_type, v.config) })),
      ], [savedViews]);

      const [activeKey, setActiveKey] = useState(() => {
        if (initialView) return VT[initialView] ? 'builtin:' + initialView : initialView;
        return lsGet(scopeKey, null) || null;
      });
      useEffect(() => { // scope change → restore that scope's remembered view
        setActiveKey(initialView ? (VT[initialView] ? 'builtin:' + initialView : initialView) : lsGet(scopeKey, null));
      }, [scopeKey]);
      ...
      const tab = tabs.find(t => t.key === activeKey)
        ...
        || tabs[0];
      const selectTab = (key) => { setActiveKey(key); lsSet(scopeKey, key); setSelected(new Set()); };
      ...
      const groupPop = usePop(), subPop = usePop(), colPop = usePop(), filtPop = usePop(), viewPop = usePop();
      ...
      const filterJson = JSON.stringify((() => {
        ...
        f.top_level = cfg.show_subtasks === 'collapsed';
      ...
      const { rows: rawRows, total, loading, error, reload, setRows } = useTasks(filter, { enabled: !!store.ready });
      ...
      const createView = async (type, flags) => {
        viewPop.close();
        const same = savedViews.filter(v => v.view_type === type).length;
        const v = await upsertView({ scope_type: sc.type, scope_id: scopeId, name: VT[type].label + (same ? ' ' + (same + 1) : ''), view_type: type,
          config: defaultConfigFor(type), is_private: !!flags.is_private, is_pinned: !!flags.is_pinned, is_default: false, is_protected: false, position: savedViews.length + 1 }, 'View created');
      ...
      const renameView = (name) => tab.view && upsertView({ ...tab.view, name }, 'View renamed');
      const flagView = (k, val) => tab.view && upsertView({ ...tab.view, [k]: val });
      const deleteView = async () => {
        if (!tab.view || !window.confirm('Delete the view “' + tab.name + '”?')) return;
        try { await TaskAPI.viewDelete(tab.view.id); toast('View deleted'); setCustomizing(false); selectTab('builtin:' + tab.type); }
      ...
      const columns = useMemo(() => (cfg.columns || []).filter(c => colMeta(c, fieldById) && !(singleClient && c === 'client' && viewType === 'list')), [cfg.columns, fieldById, singleClient, viewType]);
      ...
      const vc = useMemo(() => ({
        store, me, scope: sc, cfg, setCfg, rows: rawRows || [], setRows, reload, toast, update, setAssignees, optimistic, moveToGroup,
      ...
      }), [store, me, sc.type, sc.clientId, sc.listId, cfg, setCfg, rawRows, setRows, reload, toast, update, setAssignees, optimistic, moveToGroup,
        selected, toggleSel, selectMany, collapsedSubs, toggleCollapsedSub, childrenOf, singleClient, fieldById, availableFields, columns,
        presetFor, createTask, hasFilters, clearFilters, quickIds]);
      ...
      const showSub = viewType !== 'calendar' && viewType !== 'workload';
      ...
      const firstLoad = loading && !rawRows;
      ...
      return h`<div class="cv-root" data-view=${viewType}>
        ...
        <div class="cv-toolbar" role="toolbar" aria-label="View options">
          ...
          ${!compact && sc.type !== 'my' && h`<span style=${{ display: 'inline-flex', alignItems: 'center', paddingLeft: 8 }} role="group" aria-label="Filter by assignee">
          ...
          <${AddTaskSplit} allowPost=${allowPost} onTask=${() => createTask()} onPost=${() => createTask({ type: 'post' })}/>
        </div>
