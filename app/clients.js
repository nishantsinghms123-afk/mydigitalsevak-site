...
function ScopeTab({c,currentUser,showToast}){
  ...
  return h`<div>
    ...
    // ── SOPTab ──
// ── ClientSopDocsTab ── Brand SOP + Content Playbook live in Docs (docs.js, migration 097).
// The original six sop_* fields stay reachable behind a toggle, and are the whole tab when docs.js isn't loaded.
function ClientSopDocsTab({c,onUpdate,showToast,currentUser}){
  const[kind,setKind]=useState('sop');
  const[showLegacy,setShowLegacy]=useState(false);
  if(!ClientDocTab)return h`<${SOPTab} c=${c} onUpdate=${onUpdate} showToast=${showToast}/>`;
  const seg=(k,label,icon)=>h`<button type="button" role="tab" aria-selected=${kind===k} onClick=${()=>{setKind(k);setShowLegacy(false);}}
    style=${{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 12px',fontSize:13,fontWeight:kind===k?600:500,border:'none',borderRadius:6,cursor:'pointer',background:kind===k?'var(--cu-bg, #fff)':'transparent',color:kind===k?'var(--cu-t1, var(--t1))':'var(--cu-t2, var(--t2))',boxShadow:kind===k?'0 1px 2px rgba(0,0,0,.08)':'none'}}>
    <i class=${'ti '+icon} style=${{fontSize:14}}></i>${label}</button>`;
  return h`<div style=${{marginTop:12}}>
    <div style=${{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',marginBottom:12}}>
      <div role="tablist" aria-label="Client documents" style=${{display:'inline-flex',gap:2,padding:3,borderRadius:8,background:'var(--cu-bg3, var(--bg2))',border:'1px solid var(--cu-bd, var(--bd))'}}>
        ${seg('sop','Brand SOP','ti-list-check')}${seg('playbook','Content playbook','ti-book')}
      </div>
      ${kind==='sop'&&h`<button type="button" class="btn-sec" style=${{marginLeft:'auto',fontSize:12,padding:'5px 10px'}} onClick=${()=>setShowLegacy(v=>!v)}>
        <i class=${'ti '+(showLegacy?'ti-file-text':'ti-forms')}></i>${showLegacy?'Back to the SOP doc':'Original SOP fields'}</button>`}
    </div>
    ${showLegacy&&kind==='sop'
      ?h`<${SOPTab} c=${c} onUpdate=${onUpdate} showToast=${showToast}/>`
      :h`<${ClientDocTab} key=${c.id+':'+kind} client=${c} kind=${kind} currentUser=${currentUser} showToast=${showToast}/>`}
  </div>`;
}

// ── ClientContentTab ── the client's content list in the unified views engine (calendar by default),
// with the classic ContentCalendar (CSV import, AI planner, bulk tools) one click away.
function ClientContentTab({client,currentUser,showToast}){
  const store=useTaskStore?useTaskStore():null;
  const lists=store&&Array.isArray(store.lists)?store.lists:[];
  const contentList=lists.find(l=>l.client_id===client.id&&l.system_key==='content')||null;
  const[mode,setMode]=useState(()=>{try{return localStorage.getItem('ams_client_content_mode')||'views';}catch(_){return 'views';}});
  const pick=(m)=>{setMode(m);try{localStorage.setItem('ams_client_content_mode',m);}catch(_){}};
  const storeLoading=!!(TaskViews&&store&&store.ready===false&&!store.error);
  const canViews=!!(TaskViews&&contentList);
  const effective=canViews&&mode==='views'?'views':(storeLoading&&mode==='views'?'loading':'classic');
  const seg=(k,label,icon)=>h`<button type="button" aria-pressed=${effective===k||(k==='views'&&effective==='loading')} onClick=${()=>pick(k)}
    style=${{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 12px',fontSize:13,fontWeight:mode===k?600:500,border:'none',borderRadius:6,cursor:'pointer',background:mode===k?'var(--cu-bg, #fff)':'transparent',color:mode===k?'var(--cu-t1, var(--t1))':'var(--cu-t2, var(--t2))',boxShadow:mode===k?'0 1px 2px rgba(0,0,0,.08)':'none'}}>
    <i class=${'ti '+icon} style=${{fontSize:14}}></i>${label}</button>`;
  return h`<div style=${{marginTop:12}}>
    ${(canViews||storeLoading)&&h`<div style=${{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',marginBottom:12}}>
      <div style=${{display:'inline-flex',gap:2,padding:3,borderRadius:8,background:'var(--cu-bg3, var(--bg2))',border:'1px solid var(--cu-bd, var(--bd))'}}>
        ${seg('views','Views','ti-layout-kanban')}${seg('classic','Classic tools','ti-calendar-month')}
      </div>
      <span style=${{fontSize:12,color:'var(--cu-t3, var(--t3))'}}>${mode==='views'?'List, board, calendar, table, Gantt — switch to Classic tools for CSV import and the AI planner.':'CSV import, AI planner and bulk scheduling.'}</span>
    </div>`}
    ${effective==='loading'?h`<div style=${{display:'flex',flexDirection:'column',gap:8}}>${[0,1,2].map(i=>h`<${Skel} key=${i} h=${44}/>`)}</div>`
      :effective==='views'?h`<${TaskViews} key=${'content-'+client.id} scope=${{type:'list',listId:contentList.id,clientId:client.id,title:client.name+' · Content calendar'}} initialView="calendar" currentUser=${currentUser} showToast=${showToast}/>`
      :h`<${ContentCalendar} client=${client} currentUser=${currentUser}/>`}
  </div>`;
}
...
function ClientDetail({c,onBack,onClientUpdate,onClientDelete,currentUser,clients,onSwitch}){
  ...
  const TABS=[['overview','Overview'],...(TaskViews?[['tasks','Tasks']]:[]),['brand','Brand Profile'],...(isAdmin?[['billing','Billing']]:[]),...(isProject&&ProjectTab?[['project','Project']]:[]),['scope','Scope'],['sop','Brand SOP'],['reports','Reports'],['seo','SEO'],['calendar','Content Calendar']];
  return h`<div>
    ...
    ${tab==='sop'&&h`<${ClientSopDocsTab} c=${client} onUpdate=${updateClient} showToast=${showToast} currentUser=${currentUser}/>`}
    ...
    ${tab==='calendar'&&h`<${ClientContentTab} client=${client} currentUser=${currentUser} showToast=${showToast}/>`}
    ${tab==='tasks'&&TaskViews&&h`<div style=${{marginTop:12}}><${TaskViews} key=${client.id} scope=${{type:'client',clientId:client.id,title:client.name}} currentUser=${currentUser} showToast=${showToast}/></div>`}
    ...
    // ── AddClientModal ──
function AddClientModal({onClose,onAdd,showToast,prefill}){
  const[step,setStep]=useState(1);const[saving,setSaving]=useState(false);
  // prefill (e.g. "Convert lead to client") only fills known, non-empty string/number fields.
  const cleanPrefill=()=>{const out={};if(prefill&&typeof prefill==='object'){for(const k of ['name','industry','contact_name','contact_email','contact_phone','website','instagram_handle','brand_brief']){const v=prefill[k];if(v!=null&&String(v).trim()!=='')out[k]=typeof v==='string'?v.trim():v;}if(!out.brand_brief&&prefill.notes)out.brand_brief=String(prefill.notes).trim();}return out;};
  const[form,setForm]=useState(()=>({status:'active',retainer_type:'retainer',payment_status:'active',payment_terms:'monthly',invoice_day:1,brand_color_primary:'#1A1A1A',brand_color_secondary:'#787774',monthly_reels:0,monthly_creatives:0,monthly_carousels:0,monthly_extra:0,posting_days:'Mon,Tue,Wed,Thu,Fri,Sat',...cleanPrefill()}));
...
function ScopeReportModal({clients,onClose}){
  ...
  return h`<div class="modal" onClick=${e=>{if(e.target===e.currentTarget)onClose()}}>
    ...
    // ── ContentPlaybookCard ──
// The playbook now lives in Docs (migration 097). ContentPlaybookDoc renders that doc inline so
// there is ONE editor; the legacy ContentPlaybookModal wrote agency_settings.content_playbook
// directly and would silently overwrite — or be overwritten by — the doc on the next save.
// Falls back to the legacy card+modal only when docs.js isn't loaded.
function ContentPlaybookDoc({currentUser,showToast}){
  return h`<div style=${{marginBottom:18}}>
    <${ClientDocTab} client=${null} kind="playbook" currentUser=${currentUser} showToast=${showToast}/>
  </div>`;
}
...
function ClientsView({clients,loading,onClient,onAddClient,currentUser,showToast}){
  ...
  return h`<div>
    ...
    ${ClientDocTab
      ?h`<div style=${{marginTop:16}}><${ContentPlaybookDoc} currentUser=${currentUser} showToast=${showToast}/></div>`
