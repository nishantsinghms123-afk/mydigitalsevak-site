...
(function(){
  function buildEmployee(deps){
    const {React,h,useState,useEffect,useRef,useCallback,useMemo,supabase,SB_KEY,SB_URL,db,dbPatch,rpcCall,_loadFail,writeHash,parseHash,t,PRIORITY,Av,Toast,ThemeToggle,BottomNav,LoadErrorBanner,UrgentDTBanner,MorningDigest,HomeTab,CalendarTab,BrandsView,EmployeeView,PostPanel,DirectTasksHomePanel,isNotifForUser,notifyContent,showLocalNotification,subscribeToPush,taskOverdue,taskToday,todayISO,useAttendanceReminder,useDirectTasks,useHashRoute,usePushSubscription,NotifPanel,DesktopNotifBanner,MessagesApp,useChatUnread,useTeamChat,
            TaskAPI,taskBus,TaskPanel,CreateTaskModal,openTask,openCreateTask,HomeDashboard,MyWorkPage,InboxPage,RepliesPage,AssignedCommentsPage,TaskViews} = deps;
    const CU_SHELL = deps.CU_SHELL || {};
    // Tabs the editor/designer shell can route to (ClickUp tabs only exist when the new modules loaded).
    const CU_TABS = ['inbox','replies','assigned-comments','my-tasks','today','personal'];
    const BASE_ETABS = ['home','employee','brands','calendar','chat',...CU_TABS];
    // Feature pages registered by modules (window.AMS_EXT.pages) that this shell may show.
    const extPages = (role) => {
      const list = (window.AMS_EXT && Array.isArray(window.AMS_EXT.pages)) ? window.AMS_EXT.pages : [];
      return list.filter(p => p && p.id && p.Component && (!p.apps || p.apps.includes('tasks')) && (!p.roles || p.roles(role)))
        .slice().sort((a, b) => (a.order || 0) - (b.order || 0));
    };
    const tabsFor = (role) => [...BASE_ETABS, ...extPages(role).map(p => p.id).filter(id => !BASE_ETABS.includes(id))];
    const ETABS = BASE_ETABS;

    // A module page that throws must not take the whole shell down with it.
    class PageBoundary extends React.Component{
      constructor(props){super(props);this.state={err:null};}
      static getDerivedStateFromError(err){return{err};}
      componentDidCatch(err,info){console.error('[ams] page crashed:',this.props.label,err,info);}
      render(){
        if(!this.state.err)return this.props.children;
        return h`<div role="alert" style=${{margin:'32px auto',maxWidth:420,textAlign:'center',color:'var(--cu-t2, var(--t2))',fontSize:14}}>
          <i class="ti ti-alert-triangle" style=${{fontSize:28,color:'#e5484d'}}></i>
          <div style=${{fontWeight:600,color:'var(--cu-t1, var(--t1))',margin:'8px 0 4px'}}>${this.props.label||'This page'} ran into a problem</div>
          <div style=${{marginBottom:12}}>Reload the page if it keeps happening.</div>
          <button class="btn-sec" onClick=${()=>this.setState({err:null})}>Try again</button>
        </div>`;
      }
    }

    // Editors' Calendar: their own work in the unified views engine, or every brand's classic content calendar.
    function MyCalendarTab({user,showToast}){
      const[mode,setMode]=useState(()=>{try{return localStorage.getItem('ams_editor_cal_mode')||'mine';}catch(_){return 'mine';}});
      const pick=(m)=>{setMode(m);try{localStorage.setItem('ams_editor_cal_mode',m);}catch(_){}};
      const eff=TaskViews?mode:'brands';
      const seg=(k,label,icon)=>h`<button type="button" aria-pressed=${eff===k} onClick=${()=>pick(k)}
        style=${{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 12px',fontSize:13,fontWeight:eff===k?600:500,border:'none',borderRadius:6,cursor:'pointer',background:eff===k?'var(--cu-bg, #fff)':'transparent',color:eff===k?'var(--cu-t1, var(--t1))':'var(--cu-t2, var(--t2))',boxShadow:eff===k?'0 1px 2px rgba(0,0,0,.08)':'none'}}>
        <i class=${'ti '+icon} style=${{fontSize:14}}></i>${label}</button>`;
      return h`<div>
        ${TaskViews&&h`<div style=${{display:'inline-flex',gap:2,padding:3,margin:'0 0 12px',borderRadius:8,background:'var(--cu-bg3, var(--bg2))',border:'1px solid var(--cu-bd, var(--bd))'}}>
          ${seg('mine','My work','ti-user-check')}${seg('brands','Brand calendars','ti-calendar-month')}
        </div>`}
        ${eff==='mine'
          ?h`<${TaskViews} key="my-calendar" scope=${{type:'my',title:'My calendar'}} initialView="calendar" currentUser=${user} showToast=${showToast}/>`
...
function TasksApp({user,theme,setTheme,onSignOut}){
  const ALL_TABS=tabsFor(user&&user.role_level);
  const pages=extPages(user&&user.role_level);
  const taskStore=deps.useTaskStore?deps.useTaskStore():null;
  const railPrefs=(taskStore&&(taskStore.prefs||taskStore.me_prefs))||{};
  ...
  const[tab,setTab]=useState(()=>{const r=parseHash();return ALL_TABS.includes(r.tab)?r.tab:'home';});
  // ── ClickUp shell (migration 088) ──
  const CU=!!(CU_SHELL.IconRail&&TaskAPI);
  const[openTaskId,setOpenTaskId]=useState(()=>parseHash().taskId||null);
  ...
  // Sync tab → URL hash
  useEffect(()=>{if(parseHash().tab==='task')return;writeHash({tab});},[tab]);
  // Sync URL hash → tab (back/forward, paste-in-URL)
  const route=useHashRoute();
  useEffect(()=>{
    if(route.tab==='task'){if(route.taskId)setOpenTaskId(route.taskId);return;}
    const desired=ALL_TABS.includes(route.tab)?route.tab:'home';
  ...
  // Direct tasks open in the unified TaskPanel; the legacy inbox panel is only the fallback
  // (task modules not loaded, or migration 093's task_id_for_legacy not applied yet).
  const openDT=(t)=>{
    if(!t||!t.id)return;
    setDrawerOpen(false);
    const legacy=()=>{setDtAutoOpenId(t.id);setDTInboxOpen(true);};
    if(!(CU&&TaskAPI))return legacy();
    rpcCall('task_id_for_legacy',{p_kind:'direct',p_id:t.id},{silentAuth:true})
      .then(id=>{const tid=typeof id==='string'?id:(id&&id.id);if(tid)setOpenTaskId(tid);else legacy();})
      .catch(legacy);
  };
  ...
  const openNotifTarget=async(link_type,link_id)=>{
    ...
    switch(link_type){
      ...
      case 'direct_task':{
        if(link_id)openDT({id:link_id});else{setDtAutoOpenId(null);setDTInboxOpen(true);}
        break;
      }
      case 'task':        if(link_id)setOpenTaskId(link_id);break;
      case 'timesheet':   if(ALL_TABS.includes('timesheets'))setTab('timesheets');else showToast('Timesheets are unavailable — reload the page');break;
  ...
  // ClickUp shell wiring: global task events, skin, inbox counts, shortcuts.
  const closeTask=()=>{setOpenTaskId(null);if(parseHash().tab==='task')writeHash({tab});};
  useEffect(()=>{
    const onOpen=(e)=>{const id=e&&e.detail&&e.detail.id;if(id)setOpenTaskId(id);};
    const onCreate=(e)=>setCreateInit((e&&e.detail&&e.detail.initial)||{});
    const onPost=async(e)=>{
      const cid=e&&e.detail&&e.detail.contentId;if(!cid)return;
      try{const rows=await db('content',`&id=eq.${cid}`);if(rows&&rows[0]){setOpenTaskId(null);openCard(rows[0]);}else showToast('Post not found');}
      catch(_){showToast('Could not open the post');}
    };
    window.addEventListener('ams-open-task',onOpen);window.addEventListener('ams-create-task',onCreate);window.addEventListener('ams-open-post',onPost);
    return()=>{window.removeEventListener('ams-open-task',onOpen);window.removeEventListener('ams-create-task',onCreate);window.removeEventListener('ams-open-post',onPost);};
  },[]);
  useEffect(()=>{if(!CU)return;document.body.classList.add('cu');return()=>document.body.classList.remove('cu');},[CU]);
  useEffect(()=>{
    if(!TaskAPI)return;let alive=true;
    const f=()=>{if(document.visibilityState==='hidden')return;TaskAPI.inboxCounts().then(c=>{if(alive&&c)setInboxCounts(c);}).catch(()=>{});};
    f();const i=setInterval(f,45000);return()=>{alive=false;clearInterval(i);};
  },[user.id]);
  const usePresenceHook=CU_SHELL.usePresence||(()=>{});
  usePresenceHook(CU?user:null);
  const useCuShortcuts=CU_SHELL.useGlobalShortcuts||(()=>{});
  useCuShortcuts({
    enabled:CU,
    onPalette:()=>setPaletteOpen(true),
    onCreate:()=>openCreateTask&&openCreateTask({}),
    onNav:(id)=>{if(ALL_TABS.includes(id))setTab(id);},
    onShortcuts:()=>setShortcutsOpen(true),
  });
  const cuNavigate=(target,opts={})=>{
    if(target==='member'){const m=team.find(x=>x.id===opts.memberId);if(m)openTeamChat(m);return;}
    if(target==='client'){setTab('brands');return;}
    if(target==='task'){if(opts.id||opts.taskId)setOpenTaskId(opts.id||opts.taskId);return;}
    if(target==='page'){if(opts.id&&ALL_TABS.includes(opts.id)){setTab(opts.id);setDrawerOpen(false);if(opts.params&&opts.params.length){try{window.location.hash='#/'+[opts.id,...opts.params].map(encodeURIComponent).join('/');}catch(_){}}}return;}
    if(ALL_TABS.includes(target))goTab(target);
  };
  // Modules can also ask the shell to navigate (e.g. docs → a task, planner → a page).
  useEffect(()=>{
    const onNav=(e)=>{const d=(e&&e.detail)||{};if(d.target)cuNavigate(d.target,d.opts||{});};
    window.addEventListener('ams-navigate',onNav);
    return()=>window.removeEventListener('ams-navigate',onNav);
  });
  const openInboxNotif=(n)=>{
    if(!n)return;
    if(n.id)rpcCall('notif_mark_read',{p_ids:[n.id]},{silentAuth:true}).catch(()=>{});
    if(n.link_type){openNotifTarget(n.link_type,n.link_id);return;}
    if(!notifFallbackNav(n))showToast('Nothing more to open for this notification');
  };
  ...
  if(CU){
    const {IconRail,SideNav,SideNavItem,SideNavGroup,TopBar,CreateButton,CommandPalette,ProfileMenu,ShortcutsModal}=CU_SHELL;
    const homePages=pages.filter(p=>p.section==='home');
    const railPages=pages.filter(p=>p.section==='rail');
    const morePages=pages.filter(p=>p.section!=='home'&&p.section!=='rail');
    const HOME_TABS=['home',...CU_TABS,...homePages.map(p=>p.id)];
    const railActive=HOME_TABS.includes(tab)?'home':morePages.some(p=>p.id===tab)?'more':tab;
    const inboxN=inboxCounts?inboxCounts.primary:unreadCount;
    const TITLE={home:'Home',inbox:'Inbox',replies:'Replies','assigned-comments':'Assigned comments','my-tasks':'Assigned to me',today:'Today & overdue',personal:'Personal list',calendar:'Calendar',brands:'Brands',employee:'Employee',chat:'Chat'};
    pages.forEach(p=>{if(!TITLE[p.id])TITLE[p.id]=p.label;});
    const pageBadge=(p)=>{try{return p.badge?p.badge({currentUser:user}):null;}catch(_){return null;}};
    const baseRail=[
      {id:'home',icon:'ti-home',label:'Home',badge:inboxN||null},
      {id:'calendar',icon:'ti-calendar-month',label:'Calendar'},
      {id:'brands',icon:'ti-building-store',label:'Brands'},
      {id:'employee',icon:'ti-calendar-stats',label:'Employee'},
      ...(MessagesApp?[{id:'chat',icon:'ti-message-2',label:'Chat',badge:chatUnread>0?chatUnread:null}]:[]),
      ...railPages.map(p=>({id:p.id,icon:p.icon,label:p.label,badge:pageBadge(p)})),
    ];
    const railItems=CU_SHELL.applyRailPrefs?CU_SHELL.applyRailPrefs(baseRail,railPrefs):baseRail;
    const activePage=pages.find(p=>p.id===tab)||null;
    const crumbs=HOME_TABS.includes(tab)&&tab!=='home'?[{label:'Home',onClick:()=>goTab('home')},{label:TITLE[tab]}]:[{label:TITLE[tab]||'Home'}];
    const myWorkMode={'my-tasks':'assigned',today:'today',personal:'personal'}[tab];
    const paletteCommands=[
      {id:'new-task',label:'Create task',icon:'ti-plus',hint:'C',run:()=>openCreateTask&&openCreateTask({})},
      {id:'go-inbox',label:'Go to Inbox',icon:'ti-inbox',run:()=>goTab('inbox')},
      {id:'go-mine',label:'Go to My tasks',icon:'ti-user-check',run:()=>goTab('my-tasks')},
      {id:'go-today',label:'Go to Today & overdue',icon:'ti-calendar-due',run:()=>goTab('today')},
      {id:'go-cal',label:'Go to Calendar',icon:'ti-calendar-month',run:()=>goTab('calendar')},
      {id:'go-brands',label:'Go to Brands',icon:'ti-building-store',run:()=>goTab('brands')},
      {id:'theme',label:'Toggle dark mode',icon:'ti-moon',run:()=>setTheme(document.documentElement.classList.contains('dark')?'light':'dark')},
      {id:'shortcuts',label:'Keyboard shortcuts',icon:'ti-keyboard',hint:'?',run:()=>setShortcutsOpen(true)},
      {id:'signout',label:'Sign out',icon:'ti-logout',run:onSignOut},
    ];
    const teamOthers=team.filter(m=>m.id!==user.id);
    return h`<div class="layout cu-layout">
      <${IconRail} items=${railItems} active=${railActive} onSelect=${(id)=>goTab(id)}
        brandMark="A" brandLabel="My Digital Sevak" onBrand=${()=>goTab('home')}
        avatar=${h`<${Av} i=${user.initials||user.name.slice(0,2).toUpperCase()} c=${user.color||'#FF00EE'} s=${32} round=${true}/>`}
        avatarLabel=${user.name} onAvatar=${(e)=>setProfileAnchor(e&&e.currentTarget?e.currentTarget:null)}/>
      <aside class=${'sidebar cu-side'+(drawerOpen?' drawer-open':'')}>
        <${SideNav} title=${HOME_TABS.includes(tab)?'Home':(TITLE[tab]||'Home')} actions=${h`<${CreateButton} currentUser=${user} onNavigate=${cuNavigate} showToast=${showToast} onAction=${(k)=>{if(k==='post')openCreateTask&&openCreateTask({type:'post'});else if(k==='task')openCreateTask&&openCreateTask({});}}/>`}>
          ${showDesktopBanner&&h`<${DesktopNotifBanner} onEnable=${enableDesktopNotifs} onDismiss=${dismissBanner}/>`}
          <${SideNavItem} icon="ti-home" label="Home" active=${tab==='home'} onClick=${()=>goTab('home')}/>
          <${SideNavItem} icon="ti-inbox" label="Inbox" count=${inboxN||null} active=${tab==='inbox'} onClick=${()=>goTab('inbox')}/>
          <${SideNavItem} icon="ti-message-reply" label="Replies" active=${tab==='replies'} onClick=${()=>goTab('replies')}/>
          <${SideNavItem} icon="ti-message-user" label="Assigned comments" active=${tab==='assigned-comments'} onClick=${()=>goTab('assigned-comments')}/>
          <${SideNavGroup} label="My tasks" storageKey="mytasks">
            <${SideNavItem} icon="ti-user-check" label="Assigned to me" active=${tab==='my-tasks'} onClick=${()=>goTab('my-tasks')}/>
            <${SideNavItem} icon="ti-calendar-due" label="Today & overdue" count=${todayCount||null} active=${tab==='today'} onClick=${()=>goTab('today')}/>
            <${SideNavItem} icon="ti-lock" label="Personal list" active=${tab==='personal'} onClick=${()=>goTab('personal')}/>
          <//>
          ${homePages.map(p=>h`<${SideNavItem} key=${p.id} icon=${p.icon} label=${p.label} count=${pageBadge(p)} active=${tab===p.id} onClick=${()=>goTab(p.id)}/>`)}
          ${morePages.length>0&&h`<${SideNavGroup} label="More" storageKey="more">
            ${morePages.map(p=>h`<${SideNavItem} key=${p.id} icon=${p.icon} label=${p.label} count=${pageBadge(p)} active=${tab===p.id} onClick=${()=>goTab(p.id)}/>`)}
          <//>`}
          ${MessagesApp&&teamOthers.length>0&&h`<${SideNavGroup} label="Direct messages" storageKey="dms">
            ${teamOthers.map(m=>h`<${SideNavItem} key=${m.id} label=${m.name} avatar=${h`<${Av} i=${m.initials||m.name.slice(0,2).toUpperCase()} c=${m.color||'#999'} s=${20} round=${true}/>`} count=${dmUnread[m.id]>0?dmUnread[m.id]:null} onClick=${()=>openTeamChat(m)}/>`)}
          <//>`}
        <//>
      </aside>
      <div class="main">
        <${UrgentDTBanner} tasks=${directTasks} currentUser=${user} onOpen=${openDT}/>
        <${TopBar} crumbs=${crumbs} onMenu=${()=>setDrawerOpen(true)} onSearch=${()=>setPaletteOpen(true)}
          right=${h`${CU_SHELL.TimerPill&&h`<${CU_SHELL.TimerPill} currentUser=${user} showToast=${showToast}/>`}<button class="cu-tb-ibtn" title="Notifications" aria-label="Notifications" onClick=${()=>setShowNotifs(true)}><i class="ti ti-bell"></i>${unreadCount>0&&h`<span class="cu-tb-dot"></span>`}</button>`}/>
        ${tab==='chat'&&MessagesApp
        ?h`<div style=${{flex:1,minHeight:0,display:'flex'}}><${MessagesApp} currentUser=${user}/></div>`
        :activePage&&activePage.fullHeight
        ?h`<div style=${{flex:1,minHeight:0,display:'flex',flexDirection:'column'}}><${PageBoundary} key=${activePage.id} label=${activePage.label}><${activePage.Component} currentUser=${user} clients=${clientsArr} team=${team} onNavigate=${cuNavigate} showToast=${showToast} params=${route.params||[]} app="tasks"/><//></div>`
        :h`<div class="scroll">
          <${LoadErrorBanner} msg=${loadErr} onRetry=${()=>{setLoadErr(null);setLoading(true);loadTasks();}}/>
          ${tab==='home'?(HomeDashboard?h`<${HomeDashboard} currentUser=${user} clients=${clientsArr} onNavigate=${cuNavigate} showToast=${showToast}/>`:h`<${HomeTab} user=${user} clients=${clients} team=${[]} allTasks=${tasks} loading=${loading} onCard=${openCard} onPriChange=${updatePriority} directTasks=${directTasks} onOpenDTInbox=${openDTInbox}/>`)
          :tab==='inbox'&&InboxPage?h`<${InboxPage} currentUser=${user} onOpenNotif=${openInboxNotif} showToast=${showToast}/>`
          :tab==='replies'&&RepliesPage?h`<${RepliesPage} currentUser=${user} showToast=${showToast}/>`
          :tab==='assigned-comments'&&AssignedCommentsPage?h`<${AssignedCommentsPage} currentUser=${user} showToast=${showToast}/>`
          :myWorkMode&&MyWorkPage?h`<${MyWorkPage} key=${tab} currentUser=${user} mode=${myWorkMode} showToast=${showToast}/>`
          :activePage?h`<${PageBoundary} key=${activePage.id} label=${activePage.label}><${activePage.Component} currentUser=${user} clients=${clientsArr} team=${team} onNavigate=${cuNavigate} showToast=${showToast} params=${route.params||[]} app="tasks"/><//>`
          :tab==='calendar'?h`<${MyCalendarTab} user=${user} showToast=${showToast}/>`
          :tab==='brands'?h`<${BrandsView} showToast=${showToast}/>`
      ...
      ${openTaskId&&TaskPanel&&h`<${TaskPanel} key=${openTaskId} taskId=${openTaskId} onClose=${closeTask} showToast=${showToast}/>`}
      ${createInit&&CreateTaskModal&&h`<${CreateTaskModal} initial=${createInit} onClose=${()=>setCreateInit(null)} onCreated=${()=>{setCreateInit(null);showToast('Task created');}} showToast=${showToast}/>`}
      <${CommandPalette} open=${paletteOpen} onClose=${()=>setPaletteOpen(false)} currentUser=${user} onNavigate=${cuNavigate} showToast=${showToast} commands=${paletteCommands}/>
      <${ShortcutsModal} open=${shortcutsOpen} onClose=${()=>setShortcutsOpen(false)}/>
      ${profileAnchor&&h`<${ProfileMenu} anchor=${profileAnchor} open=${true} onClose=${()=>setProfileAnchor(null)} currentUser=${user} theme=${theme} setTheme=${setTheme} onSignOut=${onSignOut} onNavigate=${cuNavigate} showToast=${showToast} onOpenSettings=${()=>{setProfileAnchor(null);goTab('employee');}} onShortcuts=${()=>{setProfileAnchor(null);setShortcutsOpen(true);}}/>`}
      ...
      <${BottomNav} tab=${tab} items=${[{id:'home',ic:'ti-home',lb:'Home'},{id:'today',ic:'ti-calendar-due',lb:'Today'},...(MessagesApp?[{id:'chat',ic:'ti-message-2',lb:'Chat'}]:[])]} onNav=${goTab} onNotifs=${()=>goTab('inbox')} unreadCount=${inboxN} notifsActive=${tab==='inbox'}/>
