...
(function(){
  function buildEmployee(deps){
    ...
    const ETABS = ['home','employee','brands','calendar','chat',...CU_TABS];
...
function TasksApp({user,theme,setTheme,onSignOut}){
  ...
  const[tab,setTab]=useState(()=>{const r=parseHash();return ETABS.includes(r.tab)?r.tab:'home';});
  ...
  useEffect(()=>{
    ...
    const desired=ETABS.includes(route.tab)?route.tab:'home';
  ...
  useCuShortcuts({
    ...
    onNav:(id)=>{if(ETABS.includes(id))setTab(id);},
  ...
  const cuNavigate=(target,opts={})=>{
    ...
    if(ETABS.includes(target))goTab(target);
  };
  ...
  if(CU){
    ...
    const HOME_TABS=['home',...CU_TABS];
    const railActive=HOME_TABS.includes(tab)?'home':tab;
    ...
    return h`<div class="layout cu-layout">
      ...
      <aside class=${'sidebar cu-side'+(drawerOpen?' drawer-open':'')}>
        <${SideNav} title=${HOME_TABS.includes(tab)?'Home':(TITLE[tab]||'Home')} actions=${h`<${CreateButton} currentUser=${user} onAction=${(k)=>{if(k==='post')openCreateTask&&openCreateTask({type:'post'});}}/>`}>
      ...
      <div class="main">
        ...
        <${TopBar} crumbs=${crumbs} onMenu=${()=>setDrawerOpen(true)} onSearch=${()=>setPaletteOpen(true)}
          right=${h`<button class="cu-tb-ibtn" title="Notifications" aria-label="Notifications" onClick=${()=>setShowNotifs(true)}><i class="ti ti-bell"></i>${unreadCount>0&&h`<span class="cu-tb-dot"></span>`}</button>`}/>
        ...
        :h`<div class="scroll">
          ...
          :tab==='calendar'?h`<${CalendarTab} user=${user} showToast=${showToast}/>`
      ...
      <${CommandPalette} open=${paletteOpen} onClose=${()=>setPaletteOpen(false)} currentUser=${user} onNavigate=${cuNavigate} commands=${paletteCommands}/>
      <${ShortcutsModal} open=${shortcutsOpen} onClose=${()=>setShortcutsOpen(false)}/>
      ${profileAnchor&&h`<${ProfileMenu} anchor=${profileAnchor} open=${true} onClose=${()=>setProfileAnchor(null)} currentUser=${user} theme=${theme} setTheme=${setTheme} onSignOut=${onSignOut} onOpenSettings=${()=>{setProfileAnchor(null);goTab('employee');}} onShortcuts=${()=>{setProfileAnchor(null);setShortcutsOpen(true);}}/>`}
