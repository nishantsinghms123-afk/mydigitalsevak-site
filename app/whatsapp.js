...
(function () {
  function buildWhatsApp(deps) {
    ...
    const ERR = {
      ...
      bad_types: 'Pick at least one kind of task to remind people about',
      bad_lead_minutes: 'Send the reminder between 5 minutes and 24 hours before a task is due',
      bad_cap: 'The daily limit must be between 1 and 200 messages',
    };
    ...
    const KIND_META = {
      ...
      upcoming: { label: 'Upcoming', color: '#0e7490' },
    ...
    // =========================================================================
    // Upcoming task reminders (migration 094) — admin edits, manager read-only
    // =========================================================================
    const UPCOMING_TYPES = [
      ['task', 'Tasks'], ['post', 'Content posts'], ['direct', 'Direct tasks'], ['milestone', 'Project milestones'],
      ['seo_report', 'SEO reports'], ['ad', 'Ad tasks'], ['meeting', 'Meetings'], ['request', 'Requests'], ['lead', 'Leads'],
    ];
    const ALL_UPCOMING = UPCOMING_TYPES.map(t => t[0]);
    const LEAD_PRESETS = [[15, '15 minutes'], [30, '30 minutes'], [60, '1 hour'], [120, '2 hours'], [180, '3 hours'], [240, '4 hours'], [360, '6 hours'], [720, '12 hours'], [1440, '24 hours']];
    const clockAt = (hh, mm) => { const d = new Date(); d.setHours(hh, mm || 0, 0, 0); return d.toLocaleTimeString('en-IN', { hour: 'numeric', minute: '2-digit' }); };
    const leadExample = (mins) => { const d = new Date(); d.setHours(16, 0, 0, 0); return new Date(d.getTime() - (Number(mins) || 0) * 60000).toLocaleTimeString('en-IN', { hour: 'numeric', minute: '2-digit' }); };
    const prettyTime = (hhmmStr) => { const m = /^(\d{1,2}):(\d{2})/.exec(String(hhmmStr || '')); return m ? clockAt(Number(m[1]), Number(m[2])) : String(hhmmStr || ''); };

    function UpcomingCard({ canEdit, showToast }) {
      const [cfg, setCfg] = useState(null);
      const [form, setForm] = useState(null);
      const [err, setErr] = useState(null);
      const [saving, setSaving] = useState(false);
      const fill = (c) => {
        const x = c || {};
        setCfg(x);
        const types = arr(x.upcoming_types);
        setForm({
          enabled: x.upcoming_enabled !== false,
          before: Number(x.upcoming_timed_before_minutes) || 120,
          allday: hhmm(x.upcoming_allday_time) || '09:30',
          day_before: !!x.upcoming_day_before,
          types: types.length ? types : ALL_UPCOMING,
          cap: Number(x.upcoming_max_per_member_day) || 25,
        });
      };
      const load = useCallback(async () => {
        try { fill(await rpcCall('wa_config_get')); setErr(null); } catch (e) { setErr(e); }
      }, []);
      useEffect(() => { load(); }, [load]);

      const supported = !!(cfg && Object.prototype.hasOwnProperty.call(cfg, 'upcoming_enabled'));
      const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
      const toggleType = (t) => setForm(f => ({ ...f, types: f.types.includes(t) ? f.types.filter(x => x !== t) : f.types.concat(t) }));
      const dirty = !!form && !!cfg && (form.enabled !== (cfg.upcoming_enabled !== false)
        || Number(form.before) !== (Number(cfg.upcoming_timed_before_minutes) || 120)
        || form.allday !== (hhmm(cfg.upcoming_allday_time) || '09:30')
        || form.day_before !== !!cfg.upcoming_day_before
        || Number(form.cap) !== (Number(cfg.upcoming_max_per_member_day) || 25)
        || form.types.slice().sort().join(',') !== (arr(cfg.upcoming_types).length ? arr(cfg.upcoming_types) : ALL_UPCOMING).slice().sort().join(','));
      const save = async () => {
        const before = Number(form.before), cap = Number(form.cap);
        if (!(before >= 5 && before <= 1440)) { showToast(ERR.bad_lead_minutes); return; }
        if (!(cap >= 1 && cap <= 200)) { showToast(ERR.bad_cap); return; }
        if (form.enabled && !form.types.length) { showToast(ERR.bad_types); return; }
        if (!/^\d{2}:\d{2}$/.test(form.allday)) { showToast('Pick a valid time for all-day tasks'); return; }
        setSaving(true);
        try {
          fill(await rpcCall('wa_config_set', { p_data: {
            upcoming_enabled: form.enabled, upcoming_timed_before_minutes: before, upcoming_allday_time: form.allday,
            upcoming_day_before: form.day_before, upcoming_types: form.types, upcoming_max_per_member_day: cap,
          } }));
          showToast('Upcoming reminders saved ✓');
        } catch (e) { showToast(errText(e)); }
        setSaving(false);
      };
      const ro = !canEdit;

      return h`<${Card} icon="ti-clock-hour-4" title="Upcoming task reminders"
        sub="A WhatsApp nudge to the person a task is assigned to, shortly before it's due"
        right=${form && supported ? h`<span class="wa-row" style=${{ gap: 6, fontSize: 12, color: 'var(--cu-t2)' }}><span class=${'wa-dot' + (form.enabled ? ' on' : '')}></span>${form.enabled ? 'On' : 'Off'}</span>` : null}>
        ${err ? h`<div class="wa-note">${errText(err)}</div>` : !form ? h`<${Loading} rows=${3}/>`
          : !supported ? h`<div class="wa-empty">
              <i class="ti ti-database-cog" style=${{ fontSize: 26, display: 'block', marginBottom: 6, opacity: .5 }}></i>
              Upcoming reminders need database update 094. Once an admin applies it, the settings appear here.
            </div>`
          : h`<${Fragment}>
          <div class="wa-hint" style=${{ marginBottom: 12, lineHeight: 1.55 }}>
            Everyone assigned to an open task gets <b>one</b> WhatsApp before it's due — a reminder, not a chase.
            The same reminder always lands in their app inbox too, so people without WhatsApp still see it.
          </div>
          ${!cfg.enabled ? h`<div class="wa-note">WhatsApp is switched off for this workspace under <b>Setup</b>, so nothing goes out yet. The in-app reminder still works.</div>` : null}

          <div class="wa-srow" style=${{ borderTop: 'none', paddingTop: 0 }}>
            <div>
              <div class="wa-srow-t">Send upcoming reminders</div>
              <div class="wa-srow-s">Turn this off and only assignment messages and follow-ups go out.</div>
            </div>
            <${Toggle} on=${form.enabled} label="Send upcoming reminders" disabled=${ro} onChange=${v => set('enabled', v)}/>
          </div>

          <div class="wa-sub">When it goes out</div>
          <div class="wa-srow" style=${{ borderTop: 'none', flexWrap: 'wrap' }}>
            <div style=${{ minWidth: 0, flex: '1 1 240px' }}>
              <div class="wa-srow-t">Task due at a set time</div>
              <div class="wa-srow-s">Example: a task due today at ${clockAt(16, 0)} sends at <b>${leadExample(form.before)}</b>.</div>
            </div>
            <select class="wa-in" style=${{ width: 150 }} disabled=${ro} aria-label="How long before a timed task is due"
              value=${String(form.before)} onChange=${e => set('before', Number(e.target.value))}>
              ${LEAD_PRESETS.concat(LEAD_PRESETS.some(p => p[0] === Number(form.before)) ? [] : [[Number(form.before), form.before + ' minutes']])
                .map(p => h`<option key=${p[0]} value=${String(p[0])}>${p[1]} before</option>`)}
            </select>
          </div>
          <div class="wa-srow" style=${{ flexWrap: 'wrap' }}>
            <div style=${{ minWidth: 0, flex: '1 1 240px' }}>
              <div class="wa-srow-t">Task due on a date, with no time</div>
              <div class="wa-srow-s">Sends on the due date at <b>${prettyTime(form.allday)}</b> India time.</div>
            </div>
            <input class="wa-in time" type="time" disabled=${ro} aria-label="Time of day for all-day tasks" value=${form.allday} onInput=${e => set('allday', e.target.value)}/>
          </div>
          <div class="wa-srow" style=${{ flexWrap: 'wrap' }}>
            <div style=${{ minWidth: 0, flex: '1 1 240px' }}>
              <div class="wa-srow-t">Also remind the evening before</div>
              <div class="wa-srow-s">An extra message at ${clockAt(18, 0)} the day before it's due — useful for early-morning deadlines.</div>
            </div>
            <${Toggle} on=${form.day_before} label="Remind the evening before" disabled=${ro} onChange=${v => set('day_before', v)}/>
          </div>

          <div class="wa-sub">What counts</div>
          <div class="wa-hint" style=${{ marginBottom: 8 }}>Only these kinds of work trigger an upcoming reminder.</div>
          <div class="wa-row" style=${{ gap: 6 }}>
            ${UPCOMING_TYPES.map(([key, label]) => {
              const on = form.types.includes(key);
              return h`<button key=${key} type="button" class=${'wa-btn sm' + (on ? ' pri' : '')} aria-pressed=${on} disabled=${ro}
                onClick=${() => toggleType(key)}>${on ? h`<i class="ti ti-check"></i>` : null}${label}</button>`;
            })}
          </div>
          ${form.enabled && !form.types.length ? h`<div class="wa-note" style=${{ marginTop: 10 }}>${ERR.bad_types}</div>` : null}

          <div class="wa-srow" style=${{ marginTop: 14, flexWrap: 'wrap' }}>
            <div style=${{ minWidth: 0, flex: '1 1 240px' }}>
              <div class="wa-srow-t">Most reminders one person can get in a day</div>
              <div class="wa-srow-s">A safety net on a busy day — anything past the limit is skipped, and the app inbox still has it.</div>
            </div>
            <input class="wa-in" style=${{ width: 100 }} type="number" min="1" max="200" disabled=${ro} aria-label="Daily limit per person"
              value=${form.cap} onInput=${e => set('cap', e.target.value === '' ? '' : Number(e.target.value))}/>
          </div>

          <div class="wa-hint" style=${{ marginTop: 12, lineHeight: 1.55 }}>
            A task is never reminded twice for the same slot. Move its due date and the reminder re-arms for the new time.
            Quiet hours, <b>Do not disturb</b> and each person's own "Due soon" WhatsApp setting are all respected —
            so someone who has switched WhatsApp off never gets one.
          </div>

          ${canEdit ? h`<div class="wa-row" style=${{ marginTop: 16 }}>
            <button type="button" class="wa-btn pri" disabled=${saving || !dirty} onClick=${save}>${saving ? h`<${Spin}/>` : h`<i class="ti ti-check"></i>`}Save reminders</button>
            <button type="button" class="wa-btn ghost" disabled=${saving || !dirty} onClick=${() => fill(cfg)}>Reset</button>
          </div>` : h`<div class="wa-hint" style=${{ marginTop: 14 }}>Only an admin can change these settings.</div>`}
        <//>`}
      <//>`;
    }
    ...
    function WhatsAppSettingsSection({ user, showToast }) {
      ...
      return h`<div class="wa-root">
        ...
        ${isAdmin || isManager ? h`<${Fragment}>
          ...
          <${UpcomingCard} canEdit=${isAdmin} showToast=${toast}/>
